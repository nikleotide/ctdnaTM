...
## Fix — gene names + legends now render on `ctdna_concordance_oncoprint_core()`

* **Gene names invisible on last panel.** On real data (longer gene names like
  `SMARCA4`, `RAD51B`, `BARD1`, `TERT_promoter`), the gene-column labels at
  the bottom of the last panel were being visually covered by the gene-set
  (TSG / HRR) annotation strip. Fix: draw gene names as an explicit
  `HeatmapAnnotation` stack element using `anno_text`, inserted between the
  last heatmap panel and the gene-set strip. Both now coexist with a small
  visual gap and neither can cover the other regardless of gene-name length.
* **All annotation legends missing.** The `print.ctdna_concordance_oncoprint`
  S3 method was never registered in `NAMESPACE`, so `print(out)` fell back
  to `print.default` which never invoked ComplexHeatmap's `draw()` call —
  losing all 5 legends (Alteration / Dose / RECIST / Gene sets / Source)
  and the title. Fix: `S3method(print,ctdna_concordance_oncoprint)` added
  to `NAMESPACE`; also force `annotation_legend_side = "right"` and
  `heatmap_legend_side = "right"` in the print method to guarantee
  positioning.
* **New parameter:** `show_gene_set_annotation` (default `TRUE`) — set to
  `FALSE` to suppress the bottom TSG/HRR strip entirely without losing
  gene names.
