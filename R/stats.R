...
#'
#' Vectorised: accepts a scalar or numeric vector. Values below 0.001
#' collapse to \code{"p<0.001"} to avoid pretending to more precision
#' than the underlying test provides; \code{NA} maps to
#' \code{"p=NA"}.
#'
...
#' @seealso \code{\link{ctdna_run_group_test}},
#'   \code{\link{ctdna_pairwise_ranksum}},
#'   \code{\link{ctdna_cor_test}}, \code{\link{ctdna_paired_test}}
#'   (test functions that produce the p-values this helper formats).
#' @examples
#' print(ctdna_pval_label(0.0001))          # "p<0.001"
#' print(ctdna_pval_label(0.034))           # "p=0.034"
#' print(ctdna_pval_label(c(0.85, 0.001)))  # vectorised
#' print(ctdna_pval_label(NA_real_))        # "p=NA"
#' @export
...
#'
#' Selects a two-group or multi-group test appropriate for the data
#' and runs it. Used inside plot functions to compute the p-value that
#' captions and legends display. \code{method = "auto"} selects
#' Kruskal-Wallis for >2 groups and Wilcoxon rank-sum for 2 groups.
#'
#' @param data Data frame.
#' @param value Numeric column name.
#' @param group Grouping column name.
#' @param method One of \code{"auto","wilcox","t","kruskal","anova","none"}.
#' @return A list with elements \code{method} (the test actually run,
#'   after \code{"auto"} is resolved), \code{name} (human-readable test
#'   label used in plot captions), \code{p} (numeric p-value or
#'   \code{NA} if the test could not be run), and \code{n} (non-missing
#'   observation count).
#' @seealso \code{\link{ctdna_pairwise_ranksum}} (all pairwise
#'   comparisons); \code{\link{ctdna_pval_label}} (format the returned
#'   p for display).
#' @examples
#' # Two groups -> Wilcoxon by default under "auto".
#' set.seed(1)
#' df <- data.frame(group = rep(c("a","b"), each = 10),
#'                   x     = c(rnorm(10), rnorm(10, mean = 1)))
#' print(ctdna_run_group_test(df, "x", "group"))
#'
#' # Force a t-test:
#' print(ctdna_run_group_test(df, "x", "group", method = "t"))
#'
#' # Three groups -> Kruskal-Wallis by default.
#' df3 <- data.frame(group = rep(c("a","b","c"), each = 10),
#'                    x = c(rnorm(10), rnorm(10, mean = 1),
#'                          rnorm(10, mean = 2)))
#' print(ctdna_run_group_test(df3, "x", "group"))
#' @export
...
#' Pairwise rank-sum comparison across >2 groups
#'
#' Runs Wilcoxon rank-sum (default) or Welch's t-test on every
#' unordered pair of groups in \code{data[[group]]}. Returns one row
#' per pair with the p-value and both group sizes. No multiple-testing
#' correction is applied on the raw output; adjust downstream with
#' \code{stats::p.adjust()} if needed.
#'
#' @param data,value,group See [ctdna_run_group_test()].
#' @param method \code{"wilcox"} (default) or \code{"t"}.
#' @return A data frame with columns \code{group1}, \code{group2},
#'   \code{p}, \code{n1}, \code{n2}. \code{NULL} if fewer than two
#'   distinct non-\code{NA} groups are present.
#' @seealso \code{\link{ctdna_run_group_test}} (single test);
#'   \code{\link{ctdna_pval_label}} (format p-values).
#' @examples
#' set.seed(1)
#' df <- data.frame(
#'   group = rep(c("a","b","c"), each = 8),
#'   x     = c(rnorm(8), rnorm(8, mean = 0.8), rnorm(8, mean = 1.5)))
#' print(ctdna_pairwise_ranksum(df, "x", "group"))
#'
#' # Welch's t on the same pairs:
#' print(ctdna_pairwise_ranksum(df, "x", "group", method = "t"))
#' @export
...
#'
#' Runs Spearman (default), Pearson, or Kendall correlation on two
#' numeric vectors. Missing values are excluded pairwise. Wraps
#' \code{stats::cor.test} with \code{exact = FALSE} and warnings
#' suppressed, so tied ranks and small samples do not error.
#'
#' @param x,y Numeric vectors.
#' @param method One of \code{"spearman"} (default), \code{"pearson"},
#'   \code{"kendall"}. \code{"none"} short-circuits to a
#'   no-computation return.
#' @return A list with \code{method}, \code{name} (human-readable
#'   label), \code{rho} (the correlation estimate),
#'   \code{p} (two-sided p-value), \code{n} (non-\code{NA} pair
#'   count). Non-computable inputs (\code{n < 3} or \code{method =
#'   "none"}) return \code{rho = NA} and \code{p = NA}.
#' @seealso \code{\link{ctdna_paired_test}},
#'   \code{\link{ctdna_run_group_test}},
#'   \code{\link{ctdna_pval_label}}.
#' @examples
#' set.seed(1)
#' x <- runif(30)
#' y <- 2 * x + rnorm(30, sd = 0.3)
#' print(ctdna_cor_test(x, y))                       # Spearman (default)
#' print(ctdna_cor_test(x, y, method = "pearson"))
#' print(ctdna_cor_test(x, y, method = "kendall"))
#' @export
...
#' Paired test (baseline vs each timepoint)
#'
#' Paired or unpaired Wilcoxon signed-rank / t-test for two equal-length
#' numeric vectors of paired observations. \code{"paired_wilcox"}
#' (default) and \code{"paired_t"} treat elements pairwise; \code{"wilcox"}
#' and \code{"t"} drop the pairing (used when the caller wants the
#' same catalog string across paired and unpaired plots).
#'
#' @param x,y Numeric vectors of equal length (paired observations).
#' @param method One of \code{"paired_wilcox"} (default),
#'   \code{"paired_t"}, \code{"wilcox"}, \code{"t"}. \code{"none"}
#'   short-circuits.
#' @return A list with \code{method} (as supplied), \code{name}
#'   (human-readable label), \code{p} (two-sided p-value), \code{n}
#'   (non-\code{NA} pair count). Returns \code{p = NA} when the
#'   underlying test errors or \code{n < 2}.
#' @seealso \code{\link{ctdna_run_group_test}} (independent groups);
#'   \code{\link{ctdna_cor_test}}; \code{\link{ctdna_pval_label}}.
#' @examples
#' # Simulate baseline / on-treatment paired measurements.
#' set.seed(1)
#' baseline <- runif(20, min = 5, max = 15)
#' post     <- baseline * runif(20, min = 0.2, max = 0.9)  # decreases
#' print(ctdna_paired_test(baseline, post))                # paired Wilcoxon
#' print(ctdna_paired_test(baseline, post, method = "paired_t"))
#'
#' # Unpaired variant on the same data:
#' print(ctdna_paired_test(baseline, post, method = "wilcox"))
#' @export
...
#'
#' Unweighted Cohen's kappa on a square (or squarable) contingency
#' table whose row and column categories are meant to match. The
#' function trims to the intersection of \code{rownames(tab)} and
#' \code{colnames(tab)} before computing, so a nominal-agreement table
#' with different-but-overlapping labels is handled correctly.
#'
#' @param tab Square table or matrix. Row and column names should
#'   name the same set of categories (extras are dropped).
#' @return Numeric scalar: Cohen's kappa in \code{[-1, 1]}. Returns
#'   \code{NA_real_} when the table is empty or the expected agreement
#'   is exactly 1 (in which case kappa is undefined).
#' @seealso \code{\link{ctdna_weighted_kappa}} (ordinal / weighted
#'   variant); \code{\link{ctdna_concordance_test}} (bundled
#'   agreement summary).
#' @examples
#' # Perfect agreement between two 3-level raters.
#' tab <- as.table(matrix(c(10, 0, 0,
#'                          0, 12, 0,
#'                          0, 0, 8), nrow = 3, byrow = TRUE,
#'                        dimnames = list(rater1 = c("PR","SD","PD"),
#'                                        rater2 = c("PR","SD","PD"))))
#' print(ctdna_cohen_kappa(tab))     # 1
#'
#' # Some disagreement:
#' tab2 <- as.table(matrix(c(8, 2, 0,
#'                           1, 10, 1,
#'                           0, 1, 7), nrow = 3, byrow = TRUE,
#'                        dimnames = list(rater1 = c("PR","SD","PD"),
#'                                        rater2 = c("PR","SD","PD"))))
#' print(ctdna_cohen_kappa(tab2))
#' @export
...
# Cramer's V (association effect size) from a contingency table.
...
#'
#' Linear or quadratic weighted kappa on a square contingency table
#' whose row and column categories are the same ordinal set (ordered
#' the same way). Weights penalise disagreements between distant
#' categories more than adjacent-category disagreements. Quadratic
#' weighting is the RECIST convention.
#'
#' @param tab A contingency table (matching row/col ordinal
#'   categories, in the intended order).
#' @param weights \code{"quadratic"} (default; distance squared) or
#'   \code{"linear"} (distance).
#' @return Weighted kappa in \code{[-1, 1]}, or \code{NA_real_} if the
#'   table has fewer than two matching categories, is empty, or the
#'   expected agreement is exactly 1.
#' @seealso \code{\link{ctdna_cohen_kappa}} (unweighted / nominal);
#'   \code{\link{ctdna_concordance_test}}.
#' @examples
#' # Perfect ordinal agreement -> 1.
#' tab <- as.table(matrix(c(10, 0, 0, 0,
#'                          0, 12, 0, 0,
#'                          0, 0, 8, 0,
#'                          0, 0, 0, 6), nrow = 4, byrow = TRUE,
#'                        dimnames = list(
#'                          rater1 = c("CR","PR","SD","PD"),
#'                          rater2 = c("CR","PR","SD","PD"))))
#' print(ctdna_weighted_kappa(tab))                  # 1
#' print(ctdna_weighted_kappa(tab, weights = "linear"))
#'
#' # Adjacent-category disagreement is penalised less than far-apart
#' # disagreement:
#' tab2 <- as.table(matrix(c(8, 2, 0, 0,
#'                            2, 8, 2, 0,
#'                            0, 2, 6, 0,
#'                            0, 0, 0, 6), nrow = 4, byrow = TRUE,
#'                          dimnames = dimnames(tab)))
#' print(ctdna_weighted_kappa(tab2))
#' @export
...
#'
#' Bundles the three concordance measures most commonly reported on a
#' rater-agreement or modality-agreement contingency table: raw
#' percent agreement, Cohen's kappa, and (for 2x2 tables only) the
#' McNemar test for marginal-homogeneity of paired binary outcomes.
#' The n reported is the table total.
#'
#' @param tab Contingency table (rows and columns naming the same
#'   category set).
#' @param method One of \code{"both"} (default; kappa + agreement +
#'   McNemar when applicable), \code{"kappa"} (only Cohen's kappa),
#'   \code{"agreement"} (only proportion agreement),
#'   \code{"mcnemar"} (only McNemar; requires 2x2), or
#'   \code{"none"} (short-circuit).
#' @return A list with numeric components. \code{$method} echoes the
#'   argument; \code{$agreement} is the proportion of on-diagonal
#'   subjects; \code{$kappa} is unweighted Cohen's kappa; \code{$mcnemar_p}
#'   is the McNemar two-sided p (\code{NA_real_} for non-2x2 or when
#'   \code{method} excludes it); \code{$n} is the table total.
#' @seealso \code{\link{ctdna_cohen_kappa}},
#'   \code{\link{ctdna_weighted_kappa}} (ordinal variant);
#'   \code{\link{ctdna_run_group_test}} (independent-groups
#'   comparisons).
#' @examples
#' # 3-level (non-2x2): kappa + agreement, no McNemar.
#' tab <- as.table(matrix(c(8, 2, 0,
#'                          1, 10, 1,
#'                          0, 1, 7), nrow = 3, byrow = TRUE,
#'                        dimnames = list(rater1 = c("PR","SD","PD"),
#'                                        rater2 = c("PR","SD","PD"))))
#' print(ctdna_concordance_test(tab))
#'
#' # 2x2: McNemar is included.
#' tab2 <- as.table(matrix(c(30, 5,
#'                            8, 27), nrow = 2, byrow = TRUE,
#'                          dimnames = list(assay1 = c("pos","neg"),
#'                                          assay2 = c("pos","neg"))))
#' print(ctdna_concordance_test(tab2))
#' @export
...
.resolve_col <- function(x, df, what = "column") {
  # v0.79.0: a length > 1 vector is a common and understandable mistake -- the
  # user wants a second dimension. Point them at the argument that provides
  # one instead of emitting a generic type error.
  if (is.character(x) && length(x) > 1L)
    stop(sprintf(
      paste0("`%s` must be a single column. You passed %d: %s.\n",
             "  For a second dimension use one of:\n",
             "    subgroup_by = \"%s\"   (colour-coded within each %s group)\n",
             "    facet       = \"%s\"   (separate panels; `wrap` is an alias)\n",
             "  e.g. %s = \"%s\", subgroup_by = \"%s\""),
      what, length(x), paste(sprintf('"%s"', x), collapse = ", "),
      x[2], what, x[2], what, x[1], x[2]), call. = FALSE)
  ...
  # v0.78.1: final fallback -- ignore case AND separators, so any of
  # `indication`, `Indication`, `INDICATION`, `cancer_type`, `Cancer-Type`,
  # `cancer type` resolve to whichever spelling the data actually uses.
  nk <- .ctdna_norm_ind(names(df))
  ni <- which(nk == .ctdna_norm_ind(x))
  if (length(ni) >= 1L) return(names(df)[ni[1]])
  if (!is.null(hit) && is.character(hit) && length(hit) == 1L) {
    nh <- which(nk == .ctdna_norm_ind(hit))
    if (length(nh) >= 1L) return(names(df)[nh[1]])
  }
