# ============================================================================
# ctdna_concordance_oncoprint() -- ctDNA vs tissue paired oncoprint (v0.75.10)
# ----------------------------------------------------------------------------
...
# match ctdna_oncoprint() byte-for-byte.
# ============================================================================
...
#'
#' Transposed oncoprint pairing each patient's ctDNA calls with their
...
#'
#' Full annotation surface: \code{annotations} (or \code{patient_data})
#' render clinical strips at the left; \code{show_freq_bar} adds a per-gene
#' prevalence bar at the top; \code{alterations} filters alteration types;
#' \code{indications} restricts the cohort.
#'
#' @param prep A \code{ctdna_prep} object from \code{ctdna_prepare()}.
#' @param tissue_df data.frame with at least Patient_ID, Gene, Variant_type
#'   (e.g. \code{prep$dnaseq} or a WES call frame). Optionally an
#'   \code{Alteration_class} column overrides the built-in classifier.
#' @param gene_sets Named list of gene vectors. \code{NULL} (default) uses
#'   the union of every gene called in either data source. Set names are
#'   used by \code{sort_genes = "within_set"}.
#' @param filter_scheme Optional variant-filter scheme(s) applied to
#'   \code{prep$variants} before analysis.
#' @param visit Visit filter for ctDNA. \code{"baseline"} (default)
#'   expands to \code{c("C1D1","Cycle 1 Day 1","Baseline","Screening")}.
#'   Use \code{"all"} or a specific visit name to override.
#' @param wrap Optional column in \code{prep$clinical} to facet by
#'   (e.g. \code{"Indication"}).
#' @param indications Optional character vector of Indication values to
#'   restrict the cohort to. \code{NULL} (default) = no filter.
#' @param patient_data Optional per-patient annotation data.frame keyed by
#'   Patient_ID. Overrides \code{annotations} if both are supplied.
#' @param annotations Character vector of column names in \code{prep$clinical}
#'   to render as annotation strips beside each patient. Each column gets
#'   its own legend (requires the \code{ggnewscale} package).
#' @param show_freq_bar Show the per-gene prevalence bar at the top
#'   (dodged bars, ctDNA vs Tissue). Default \code{TRUE}.
#' @param alterations Optional character vector restricting which alteration
#'   classes are drawn. Must be a subset of Focal_Amp / Amp /
#'   Homozygous_Del / LOH / Missense / Truncating / InFrame / Promoter /
#'   LGR / Fusion / Other.
#' @param sort_genes Gene column ordering. \code{"global"} (default) sorts
#'   by combined prevalence; \code{"within_set"} sorts within each
#'   \code{gene_sets} group; \code{"none"} keeps input order.
#' @param max_patients_per_panel Cap patients per facet by top-N combined
#'   burden. Default \code{25}.
#' @param show_patient_names \code{TRUE} / \code{FALSE} / \code{NA} (auto).
#' @param sort_by Patient row ordering within each facet.
#' @param title,subtitle,caption Plot chrome.
#' @param legend_position Passed to \code{theme(legend.position = ...)}.
#'
#' @return A composed \code{ggplot} (via patchwork) with class
#'   \code{"ctdna_concordance_oncoprint"}. Add layers directly with
#'   \code{+}. Auxiliary data attached as attributes: \code{ctdna_matrix},
#'   \code{tissue_matrix}, \code{plot_dim}, \code{patients}, \code{genes},
#'   \code{long}, \code{n_dropped_by_topN}.
#'
#' @examples
#' \dontrun{
...
ctdna_concordance_oncoprint <- function(
  ...
  if (!is.list(prep) || is.data.frame(prep))
    stop("prep must be a ctdna_prep object.", call. = FALSE)
  if (!is.data.frame(tissue_df))
    stop("tissue_df must be a data.frame with Patient_ID, Gene, Variant_type.",
         call. = FALSE)
  need <- c("Patient_ID","Gene","Variant_type")
  miss <- setdiff(need, names(tissue_df))
  if (length(miss))
    stop("tissue_df is missing columns: ", paste(miss, collapse=", "),
         call. = FALSE)
  sort_by    <- match.arg(sort_by)
  sort_genes <- match.arg(sort_genes)

  # ---- indications cohort restriction --------------------------------------
  if (!is.null(indications)) {
    prep <- .ctdna_filter_prep_by_indication(prep, indications, col_hint = wrap)
    if (is.data.frame(prep$clinical) &&
        "Patient_ID" %in% names(prep$clinical))
      tissue_df <- tissue_df[as.character(tissue_df$Patient_ID) %in%
                              as.character(prep$clinical$Patient_ID), ,
                              drop = FALSE]
  }

  # ---- 1. classify + collapse per patient ---------------------------------
  cvar <- prep$variants
  if (!is.null(filter_scheme) && exists("ctdna_variant_filter",
                                          mode = "function")) {
  ...
  if (!is.null(visit) && !identical(tolower(as.character(visit)), "all") &&
    ...
    cvar <- cvar[normalise(cvar$Visit_name) %in% normalise(keep_visits),
                 , drop = FALSE]
  }
  ...
  if (!is.null(alterations)) {
    ...
    if (length(unknown))
      stop("alterations: unknown class(es): ", paste(unknown, collapse=", "),
           "\n  Valid: ", paste(names(.co_alt_palette), collapse=", "),
           call. = FALSE)
  ...
  if (!is.null(wrap)) {
    if (!is.data.frame(prep$clinical) || !wrap %in% names(prep$clinical))
      stop("wrap column '", wrap, "' not found in prep$clinical.",
           call. = FALSE)
  ...
  if (!is.null(patient_data) && is.data.frame(patient_data) &&
      nrow(patient_data)) {
    pd <- patient_data[match(patients_all,
    ...
    if (length(ann_cols)) {
      ...
      if (!requireNamespace("ggnewscale", quietly = TRUE))
        stop("ctdna_concordance_oncoprint: annotations require the ",
              "`ggnewscale` package (one legend per annotation column). ",
              "install.packages(\"ggnewscale\")", call. = FALSE)

      ann_plot <- ggplot2::ggplot(mapping = ggplot2::aes(x = .data$Ann,
                                                           y = .data$Row))
  ...
  if (isTRUE(show_freq_bar)) {
    ...
    freq_plot <- ggplot2::ggplot(freq_df,
      ...
      ggplot2::geom_col(position = ggplot2::position_dodge(width = 0.8),
                          width = 0.7) +
      ggplot2::scale_fill_manual(values = c(ctDNA = "#2C7FB8",
                                              Tissue = "#D95F0E"),
                                   name = NULL) +
  ...
  if (!is.null(freq_plot) && !is.null(ann_plot)) {
    # Layout: freq bar in top-right, annotations bottom-left, main bottom-right
    spacer <- patchwork::plot_spacer()
    p <- patchwork::wrap_plots(spacer, freq_plot, ann_plot, main_composable,
                                nrow = 2, ncol = 2,
    ...
    p <- patchwork::wrap_plots(freq_plot, main, ncol = 1) +
      patchwork::plot_layout(heights = c(0.15, 1), guides = "collect") +
      patchwork::plot_annotation(title = title, subtitle = subtitle,
                                   caption = caption,
        theme = ggplot2::theme(plot.title = ggplot2::element_text(face = "bold")))
  } else if (!is.null(ann_plot)) {
    p <- patchwork::wrap_plots(ann_plot, main_composable, ncol = 2,
                                widths = c(ann_width, 1)) +
      patchwork::plot_layout(guides = "collect") +
      patchwork::plot_annotation(title = title, subtitle = subtitle,
                                   caption = caption,
...
#'
#' Character vector of 50 gene symbols used as a pan-cancer landscape set.
#' Composition: solid-tumor drivers (TP53, PIK3CA, KRAS, APC, PTEN, BRAF,
#' etc.), DDR / HRR genes, heme drivers, RTKs / fusion targets, chromatin
#' regulators, and indication-specific standouts.
#'
