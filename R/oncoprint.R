# =============================================================================
# v0.39.0 -- ctdna_oncoprint() internals
# =============================================================================
#
# Public entry point (ctdna_oncoprint) is defined in ctdnaTM.R. Everything
...
# v0.63.0: bottom-to-top draw order so glyphs layer as requested --
# CNV (full cell) at the bottom, then LGR (75% box), then truncating, then the
# other SNV/indels on top. Classes not listed sort to the end (drawn on top).
.oncoprint_class_zorder <- c(
  "Focal_Amp","Amp","Homozygous_Del","LOH",   # CNV -- bottom (full cell)
  "Fusion","Other",                            # other full-cell structural/misc
  "LGR",                                        # 75% box, centred
  "Truncating",                                 # thin, above LGR
  "InFrame","Missense","Promoter")              # SNV/indels -- top (thin)
...
#' \code{\link{ctdna_oncoprint}} without memorising the strings.
#'
...
#'
#' # Typical use: build a subset and pass as `alterations =` to
#' # ctdna_oncoprint(). Note that ctdna_oncoprint() takes a prep object
#' # (not the raw Infinity report) and requires a `gene_sets` argument.
...
# v0.42.0: alteration-set resolver used by ctdna_oncoprint().
...
.resolve_oncoprint_alterations <- function(alterations, flag_dots) {
  ...
  if (length(unknown) > 0L)
    stop("Unknown argument(s) passed to ctdna_oncoprint(): ",
  ...
  if (!is.null(alterations)) {
    ...
    if (length(flags) > 0L)
      warning("Both `alterations` and individual flags were passed to ",
              "ctdna_oncoprint(); using `alterations` and ignoring flags.",
              call. = FALSE)
...
.oncoprint_set2_palette <- c("#66C2A5","#FC8D62","#8DA0CB","#E78AC3",
                             ...
                             # v0.78.0: extended beyond the original eight so
                             # studies with more than 8 indications / cohorts
                             # no longer hit "Insufficient values in manual
                             # scale". The first eight are unchanged, so any
                             # figure with <= 8 levels looks identical.
...
.oncoprint_resolve_alias <- function(x, arg_name, patient_data, df,
  ...
  stop(sprintf(
    "ctdna_oncoprint: `%s = '%s'` is not a column in patient_data or df, ",
    arg_name, x),
...
.oncoprint_resolve_gene_sets <- function(df, gene_sets, gene_col) {
  ...
  resolved <- if (is.null(gene_sets)) {
    ...
    if (length(unknown))
      stop("ctdna_oncoprint: unknown gene-set name(s): ",
           paste(unknown, collapse = ", "),
    ...
    if (is.null(names(gene_sets)) || any(!nzchar(names(gene_sets))))
      stop("ctdna_oncoprint: inline `gene_sets` must be a NAMED list, ",
           "e.g. list(HRR = c('BRCA1','BRCA2')).",
           call. = FALSE)
    gene_sets
  } else {
    stop("ctdna_oncoprint: `gene_sets` must be NULL, a character vector ",
...
.oncoprint_resolve_wrap <- function(wrap, patient_data, pd_id_col, df,
  ...
  if (sum(!keep))
    message("ctdna_oncoprint: ", sum(!keep),
...
.oncoprint_build_matrix <- function(df, patient_data, pd_id_col, patient_col,
  ...
  if (nrow(global_mat) == 0L)
    stop("ctdna_oncoprint: no alterations to plot after classification.",
         call. = FALSE)
...
.oncoprint_build_legend_state <- function(top_annotations, patient_data,
  ...
  for (ann in top_annotations) {
    if (!ann %in% names(patient_data)) {
      warning("ctdna_oncoprint: annotation '", ann,
...
.oncoprint_alter_fun <- function(classes_present) {
  ...
  # v0.63.0: draw in z-order (CNV bottom -> LGR -> truncating -> SNV/indels top).
  classes_present <- classes_present[
    order(match(classes_present, .oncoprint_class_zorder), na.last = TRUE)]
  for (cl in classes_present) {
    local({
      cc <- cl; fill <- .oncoprint_alt_palette[[cc]]
      if (cc %in% .oncoprint_thin_classes) {
        af[[cc]] <<- function(x, y, w, h)               # thin centred bar
          grid::grid.rect(x, y, w - grid::unit(0.5,"mm"), h * 0.33,
                          gp = grid::gpar(fill = fill, col = NA))
      } else if (cc == "LGR") {
        af[[cc]] <<- function(x, y, w, h)               # 75% of cell, centred
          grid::grid.rect(x, y, w - grid::unit(0.5,"mm"), h * 0.75,
                          gp = grid::gpar(fill = fill, col = NA))
      } else {
        af[[cc]] <<- function(x, y, w, h)               # full cell (CNV etc.)
          grid::grid.rect(x, y,
                          w - grid::unit(0.5,"mm"), h - grid::unit(0.5,"mm"),
                          gp = grid::gpar(fill = fill, col = NA))
      }
...
.oncoprint_panel_gg <- function(global_mat, ordered_genes, row_split,
  ...
  # v0.66.0: robust to all-empty panels (every kept patient zero-burden)
  if (is.null(long) || !nrow(long))
    long <- data.frame(Patient = character(0), Gene = character(0),
...
.oncoprint_assemble_gg <- function(panels, title, subtitle, caption,
                                     legend_position) {
  if (!length(panels)) stop("ctdna_oncoprint: no ggplot panels.",
                              call. = FALSE)
  ...
  if (!requireNamespace("patchwork", quietly = TRUE)) {
    message("ctdna_oncoprint: patchwork not installed; the ggplot ",
...
.oncoprint_autodetect_cols <- function(df, patient_col, gene_col, variant_col) {
  ...
  if (is.na(patient_col))
    stop("ctdna_oncoprint: could not find a patient ID column in df. ",
         "Set `patient_col` explicitly.", call. = FALSE)
  if (is.na(gene_col))
    stop("ctdna_oncoprint: could not find a gene column in df. ",
         "Set `gene_col` explicitly.", call. = FALSE)
  if (is.na(variant_col))
    stop("ctdna_oncoprint: could not find a variant-type column in df. ",
