...
#'
#' Convenience wrapper around [ctdna_summary()] + [merge()]. Collapses
#' the longitudinal ctDNA frame to one row per subject (via
#' \code{ctdna_summary}) and inner-joins that summary with another
#' per-subject modality frame keyed on the subject column.
#'
#' @param ctdna_df Longitudinal ctDNA data (typically
#'   \code{prep$samples}).
#' @param other_df Modality data with the subject column.
#' @param by Join key (default \code{ctdna_opts("subject")}, usually
#'   \code{"Patient_ID"}).
#' @return Merged data frame: one row per subject present in
#'   \strong{both} inputs. Column set is the union of
#'   \code{ctdna_summary(ctdna_df)} + \code{other_df} (minus the
#'   duplicate join key).
#' @seealso \code{\link{ctdna_summary}} (the underlying per-subject
#'   collapse); \code{\link{ctdna_metric_at}} (extract a single
#'   per-subject metric at a chosen visit); the cross-modality plot
#'   family (\code{\link{ctdna_plot_vs_tmb}},
...
#'
#' # Build a per-subject TMB frame from prep$samples and join it to the
#' # ctDNA summary.
#' tmb_df <- unique(prep$samples[, c("Patient_ID", "TMB_score")])
#' merged <- ctdna_merge_modalities(prep$samples, tmb_df)
#' print(head(merged))
#' @export
...
#'   Display.
#' @param stat_position Position for statistical annotation. Default
#'   from \code{ctdna_opts("stat_position")}. Scatter correlations
#'   report in the caption, so this rarely needs changing.
#' @param legend_position ggplot legend position: \code{"right"}
#'   (default from \code{ctdna_opts("legend_position")}),
#'   \code{"bottom"}, \code{"top"}, \code{"left"}, or \code{"none"}.
#' @param xlim,ylim Optional numeric \code{c(lo, hi)} axis-limit pairs.
#'   \code{NULL} (default) uses the data range.
#' @param y_scale Y-axis transform for the expression axis.
#'   \code{"linear"} (default; expression is already
#'   \eqn{log_2(TPM + 1)} canonically), \code{"log10"}, \code{"log2"},
#'   or \code{"log"}.
...
#' @examples
#' sim  <- ctdna_make_mock_study(n_patients = 20, seed = 1)
#' # Pass rnaseq = sim$rnaseq so prepare() builds a long-format
#' # subject + gene + expression frame at prep$rnaseq.
#' prep <- ctdna_prepare(
#'   infinity_report = sim$infinity_report,
#'   adam            = list(adsl = sim$clinical),
#'   rnaseq          = sim$rnaseq,
...
#' prep <- ctdna_sample_qc(prep, verbose = FALSE)
#'
#' # ctDNA best-ratio vs PD-L1 (CD274) expression.
#' p <- ctdna_plot_vs_expression(prep$samples, prep$rnaseq, gene = "CD274")
#' print(p)
#' @export
ctdna_plot_vs_expression <- function(ctdna_df, expr_df, gene,
  ...
  ctdna_df <- .ctdna_filter_df_by_indication(ctdna_df, .ctdna_resolve_indication(indication, indications))
  # ---------------------------------------
  .stats_label <- NULL
...
#'   With \code{prep$variants} as \code{mut_df}, use
#'   \code{by = "alteration_type"} -- that maps to the
#'   \code{Variant_type} column which is present verbatim on the
#'   variants frame.
#' @param metric ctDNA metric.
#' @param recist_facet Logical. When \code{TRUE}, wrap the boxplot in
#'   a per-RECIST facet grid (using \code{scheme} to define the
#'   strata). Default \code{FALSE} = single panel.
#' @param scheme RECIST stratification scheme used when
#'   \code{recist_facet = TRUE}. One of \code{"three"} (default),
#'   \code{"two"}, \code{"four"}, \code{"four_alt"}, \code{"five"},
#'   \code{"six"}.
#' @param scales Facet-scale argument passed to
#'   \code{ggplot2::facet_wrap} when \code{recist_facet = TRUE}:
#'   \code{"fixed"} (default), \code{"free_y"}, \code{"free_x"}, or
#'   \code{"free"}.
#' @param filter_scheme Optional pre-filter applied to \code{mut_df}
#'   before joining, using the standard filter-scheme grammar. Scheme
#'   name, a \code{ctdna_filtering_scheme} object, or a list thereof.
#'   \code{NULL} (default) leaves \code{mut_df} untouched. Users
#'   typically call \code{\link{ctdna_variant_filter}} explicitly on
#'   \code{prep$variants} first, then pass the filtered frame here.
#' @param stat One of `"wilcox"` (default), `"t"`, `"kruskal"`, `"none"`.
#'   Recommended: `"wilcox"` (two groups) or `"kruskal"` (multi-state).
#' @param show_stats,title,subtitle,caption,xlab,ylab,point_size,point_alpha
#'   Display.
#' @param stat_position Position for statistical annotation. Default
#'   from \code{ctdna_opts("stat_position")}.
#' @param legend_position ggplot legend position: \code{"right"}
#'   (default from \code{ctdna_opts("legend_position")}),
#'   \code{"bottom"}, \code{"top"}, \code{"left"}, or \code{"none"}.
#' @param xlim,ylim Optional numeric axis-limit pairs. \code{ylim}
#'   accepts \code{c(lo, hi)}; \code{xlim} for a categorical x-axis
#'   accepts a character vector fixing the level order. \code{NULL}
#'   (default) = data-driven.
#' @param y_scale Y-axis transform for the ctDNA-metric axis.
#'   \code{"log10"} (default), \code{"linear"}, \code{"log2"}, or
#'   \code{"log"}.
#' @param indication Optional indication (cohort) restriction. A single string
#'   or a character vector of Indication values, e.g.
#'   \code{indication = "HNSCC"} or \code{indication = c("HNSCC","ES_SCLC")}.
...
#' prep <- ctdna_sample_qc(prep, verbose = FALSE)
#'
#' # Boxplot of ctDNA metric by alteration type in TP53 (wt vs each
#' # alteration class). Use by = "alteration_type" with prep$variants;
#' # the "mutation_status" mode requires a synthesised per-patient
#' # mut/wt column that ctdna_prepare() no longer produces.
#' p <- ctdna_plot_vs_mutation(prep$samples, prep$variants,
#'   gene = "TP53", by = "alteration_type")
#' print(p)
#' @export
...
#' @param recist_color Logical. When \code{TRUE}, colour points by
#'   RECIST (per \code{scheme}); the regression line is drawn in grey
#'   underneath. \code{FALSE} (default) uses one grey colour for all
#'   points.
#' @param scheme RECIST stratification scheme used when
#'   \code{recist_color = TRUE}. One of \code{"three"} (default),
#'   \code{"two"}, \code{"four"}, \code{"four_alt"}, \code{"five"},
#'   \code{"six"}.
#' @param stat One of `"spearman"` (default), `"pearson"`, `"kendall"`,
#'   `"none"`. Recommended: `"spearman"`.
#' @param show_stats,title,subtitle,caption,xlab,ylab,point_size,point_alpha
#'   Display.
#' @param stat_position Position for statistical annotation; passed
#'   through to the stat layer. Default from
#'   \code{ctdna_opts("stat_position")}. Scatter correlations report
#'   in the caption, so this rarely needs changing.
#' @param legend_position ggplot legend position: \code{"right"}
#'   (default from \code{ctdna_opts("legend_position")}),
#'   \code{"bottom"}, \code{"top"}, \code{"left"}, or \code{"none"}.
#' @param xlim,ylim Optional numeric \code{c(lo, hi)} axis-limit pairs.
#'   \code{NULL} (default) uses the data range. x-axis is always
#'   \code{log10} (ctDNA metric); y-axis transform is controlled by
#'   \code{y_scale}.
#' @param y_scale Y-axis transform for the TMB axis. \code{"linear"}
#'   (default), \code{"log10"}, \code{"log2"}, or \code{"log"}.
#' @param indication Optional indication (cohort) restriction. A single string
#'   or a character vector of Indication values, e.g.
#'   \code{indication = "HNSCC"} or \code{indication = c("HNSCC","ES_SCLC")}.
#'   \code{NULL} (the default) uses every indication present in the data.
...
#' prep <- ctdna_sample_qc(prep, verbose = FALSE)
#'
#' # TMB is carried on prep$samples (column TMB_score), so pass the
#' # samples frame for both the ctDNA and TMB inputs.
#' p1 <- ctdna_plot_vs_tmb(prep$samples, prep$samples)
#' print(p1)
#'
#' # Colour points by RECIST scheme.
#' p2 <- ctdna_plot_vs_tmb(prep$samples, prep$samples,
...
#' @param xlim,ylim Optional numeric \code{c(lo, hi)} axis-limit pairs.
#'   \code{NULL} (default) uses the data range.
#' @param y_scale Y-axis transform for the IHC-score axis.
#'   \code{"linear"} (default), \code{"log10"}, \code{"log2"}, or
#'   \code{"log"}.
#' @param indication Optional indication (cohort) restriction. A single string
#'   or a character vector of Indication values, e.g.
#'   \code{indication = "HNSCC"} or \code{indication = c("HNSCC","ES_SCLC")}.
...
#' @examples
#' sim  <- ctdna_make_mock_study(n_patients = 20, seed = 1)
#' # Pass ihc = sim$ihc so prep$ihc is populated for cross-modality plots.
#' prep <- ctdna_prepare(
#'   infinity_report = sim$infinity_report,
#'   adam            = list(adsl = sim$clinical),
#'   ihc             = sim$ihc,
...
#' p <- ctdna_plot_vs_ihc(prep$samples, prep$ihc, marker = "PDL1")
#' print(p)
#' @export
