...
# to one row per unique sample. Used by ctdna_qc_filter() and accessible
...
.gh_build_genomic <- function(infinity_report,
  ...
  # filter by default. Filtering moved to ctdna_qc_filter() so users
...
.prepare_gh_infinity <- function(infinity_report      = NULL,
  ...
  # infinity_report (not just QC). Used by ctdna_qc_filter() and any
...
#'   \code{\link{ctdna_filter_apply}} for alteration-level filtering
...
.ctdna_column_aliases <- function() {
  list(
    ...
    TF_change = c("TF_change","tf_change","TFChange","TumorFraction_change",
                  "TF_delta"),
    methylTF = c("methylTF","methyl_tf","methylationTF","methylation_TF",
                 "methyl_TF","mTF",
                 "Methylation_tumor_fraction_percentage",
                 "Methylation_tumor_fraction",
                 "methylation_tumor_fraction_percentage"),
    maxVAF = c("maxVAF","max_vaf","maxvaf","Max_VAF","MAX_VAF",
               "Genomic_max_VAF_percentage","Genomic_max_VAF",
               "genomic_max_vaf_percentage"),
    meanVAF = c("meanVAF","mean_vaf","meanvaf","Mean_VAF","MEAN_VAF"),
    TMB_score = c("TMB_score","tmb_score","TMB","tmb","TMB_Score","TMBScore",
                  "Tumor_Mutational_Burden"),
    Dose = c("Dose","dose","DOSE","Dose_level","dose_level","DoseLevel",
             "DOSE_LEVEL","Dose_Level",
             # v0.43.7: ARM (descriptive: "8 mg/kg GSK5764227") wins over
...
ctdna_prepare <- function(bundle               = NULL,
                          # ----- Explicit modality slots (GH Infinity defaults) -----
                          infinity_report      = NULL,
                          tf_change            = NULL,
                          panel_74_response    = NULL,
                          panel_500_response   = NULL,
  ...
  # v0.42.7: backward-compat alias. The first positional argument used to be
  # called `sim`. Renamed to `bundle` for clarity. The old `sim` name still
  # works but warns once per session.
  if (!is.null(sim)) {
    if (!is.null(bundle))
      stop("ctdna_prepare: provide either `bundle` (preferred) or `sim` ",
           "(deprecated alias), not both.", call. = FALSE)
    .ctdna_warn_once("ctdna_prepare_sim_deprecated",
                     paste0("ctdna_prepare: the `sim = ...` argument is ",
                            "DEPRECATED in v0.42.7; use `bundle = ...` instead."))
    bundle <- sim
  }

  # v0.42.8: collect the explicit modality args + any ... extension args
  modality_args <- list(
    infinity_report      = infinity_report,
    tf_change            = tf_change,
  ...
  modality_args <- modality_args[!vapply(modality_args, is.null, logical(1))]

  dots <- list(...)
  if (length(dots) > 0L) {
    if (is.null(names(dots)) || any(!nzchar(names(dots))))
      stop("ctdna_prepare: extra arguments passed via `...` must be named ",
           "(e.g. custom_modality = df).", call. = FALSE)
    # Coercion to data.frame is handled later by .ctdna_to_data_frame;
    # we don't strictly require is.data.frame() here.
    new_keys <- setdiff(names(dots), names(modality_args))
    modality_args[new_keys] <- dots[new_keys]
  }

  # v0.75.10: tree-reader spec expansion.
  # A modality slot can now be a list(loc = "dir", regex = "*.tsv"). Detect
  # that shape, call the delimited-tree reader, and replace the list with the
  # resulting data.frame. Applies to dnaseq / rnaseq (and any custom modality
  # passed via `...`). Also applies when the loc/regex live in ctdna_opts:
  # if the caller passed dnaseq = NULL but ctdna_opts("dnaseq_loc") and
  # ctdna_opts("dnaseq_regex") are set, we still trigger the reader.
  .is_tree_spec <- function(x) {
    is.list(x) && !is.data.frame(x) &&
      all(c("loc","regex") %in% names(x))
  }
  # Direct-list case
  for (mnm in intersect(c("dnaseq","rnaseq"), names(modality_args))) {
    m <- modality_args[[mnm]]
    if (.is_tree_spec(m)) {
      if (isTRUE(verbose))
        message(sprintf("ctdna_prepare: reading %s from tree at %s ...",
                         mnm, m$loc))
      modality_args[[mnm]] <- .ctdna_read_delimited_tree(
        loc = m$loc, regex = m$regex, modality = mnm, verbose = verbose)
    }
  }
  # Opts-driven case (only if the slot is not already provided)
  for (mnm in c("dnaseq","rnaseq")) {
    if (mnm %in% names(modality_args)) next   # user supplied something
    loc_k   <- paste0(mnm, "_loc")
    rgx_k   <- paste0(mnm, "_regex")
    loc_v   <- tryCatch(ctdna_opts(loc_k, default = NULL),
                         error = function(e) NULL)
    rgx_v   <- tryCatch(ctdna_opts(rgx_k, default = NULL),
                         error = function(e) NULL)
    if (!is.null(loc_v) && !is.null(rgx_v) &&
         nzchar(loc_v) && nzchar(rgx_v)) {
      if (isTRUE(verbose))
        message(sprintf("ctdna_prepare: reading %s from opts-configured tree ",
                         "%s (regex = %s) ...", mnm, loc_v, rgx_v))
      modality_args[[mnm]] <- .ctdna_read_delimited_tree(
        loc = loc_v, regex = rgx_v, modality = mnm, verbose = verbose)
    }
  }

  # v0.42.12: extract per-modality specs. Each modality slot can be:
  #   - a data frame / matrix / tibble / data.table (just the frame)
  #   - a list(data = <frame>, patient_id_col = "X", maxvaf_col = "Y", ...)
  # Returns a parallel list of $data and $renames per modality.
  modality_specs <- mapply(function(x, nm) .ctdna_extract_modality_spec(x, nm),
                            modality_args, names(modality_args),
                            SIMPLIFY = FALSE)
  modality_args  <- lapply(modality_specs, `[[`, "data")
  modality_renames <- lapply(modality_specs, `[[`, "renames")

  # ---- merge `bundle` (if any) with the explicit modality args ----
  if (!is.null(bundle)) {
    if (is.data.frame(bundle))
      stop("ctdna_prepare: you passed a single data frame positionally as ",
           "`bundle`, but the function needs to know which modality it ",
           "represents. Use a NAMED argument instead. Examples:\n",
           "  ctdna_prepare(clinical = your_df)\n",
           "  ctdna_prepare(infinity_report = your_df)\n",
           "  ctdna_prepare(infinity_report = inf_df, clinical = clin_df, ihc = ihc_df)\n",
           "See ?ctdna_prepare for the full list of expected modality keys.",
           call. = FALSE)
    if (!is.list(bundle))
      stop("ctdna_prepare: `bundle` must be a NAMED list of data frames ",
           "(was: ", class(bundle)[1], "). See ?ctdna_prepare.",
           call. = FALSE)
    if (is.null(names(bundle)) || any(!nzchar(names(bundle))))
      stop("ctdna_prepare: every element of `bundle` must be named ",
           "(e.g. `infinity_report`, `clinical`, `ihc`). See ?ctdna_prepare.",
           call. = FALSE)
    # Coerce any non-plain-data.frame entries in the bundle
    bundle <- mapply(function(x, nm) .ctdna_to_data_frame(x, paste0("bundle$", nm)),
                      bundle, names(bundle), SIMPLIFY = FALSE)
    # Bundle and explicit modality args: explicit args WIN on conflict
    keep_bundle_keys <- setdiff(names(bundle), names(modality_args))
    modality_args[keep_bundle_keys] <- bundle[keep_bundle_keys]
  }

  bundle <- modality_args

  if (length(bundle) == 0L)
    stop("ctdna_prepare: no input frames provided. Pass at least one ",
         "modality frame, e.g. ",
         "ctdna_prepare(infinity_report = inf_df, clinical = clin_df). ",
         "See ?ctdna_prepare for the full list of accepted modality slots.",
         call. = FALSE)

  # ---- v0.42.10/11/12: canonicalize column names via the alias map ----
  # User frames may use SUBJID / subject_id / etc. instead of Patient_ID.
  # Per-modality renames (from list-form specs) take highest precedence,
  # then global patient_id_col, then alias-based auto-detection.
  bundle <- mapply(function(df, nm) {
    .ctdna_canonicalize_columns(
      df, nm,
      patient_id_override = patient_id_col,
      renames             = if (!is.null(modality_renames[[nm]])) modality_renames[[nm]] else character(0),
      verbose             = verbose
    )
  }, bundle, names(bundle), SIMPLIFY = FALSE)

  # ---- v0.42.8/9: shape and duplicate diagnostics ----
  .ctdna_check_modality_shapes(bundle, strict = strict, verbose = verbose)
  .ctdna_check_duplicate_frames(bundle, strict = strict, verbose = verbose)
  ...
  if (verbose) message("Preparing study bundle for platform: ", spec$name)

  # Canonical column-name opts (used by the per-modality helpers below)
  c_subj <- .o("subject")
  c_gene <- .o("gene")
  c_expr <- .o("expression")
  ...
  # ---- ctDNA bundle (dispatch to platform handler) ----
  ctdna_keys <- c("infinity_report","tf_change","panel_74_response",
                  "panel_500_response","methylation_response")
  has_ctdna <- any(ctdna_keys %in% names(bundle))
  if (has_ctdna) {
    # v0.40.0: apply filter_scheme to the RAW infinity_report (which has
    # vendor column names like Variant_type / Gene that filter rules
    # expect) BEFORE the platform handler transforms it. In v0.39.x the
    # filter was applied post-dispatch on canonical-name frames and
    # silently matched nothing useful.
    inf_for_prep <- bundle$infinity_report
    if (!is.null(filter_scheme) && !is.null(inf_for_prep) &&
        any(c("Variant_type","Gene") %in% names(inf_for_prep))) {
      if (verbose) message(" - applying filter_scheme '", filter_scheme,
                            "' to infinity_report")
      inf_for_prep <- ctdna_filter_apply(inf_for_prep,
                                          filter_scheme = filter_scheme,
                                          verbose = verbose)
    }
    if (verbose) message(" - ctDNA + genomic panels (platform handler)")
    ctdna_out <- spec$prepare(
      infinity_report      = inf_for_prep,
      tf_change            = bundle$tf_change,
      panel_74_response    = bundle$panel_74_response,
      panel_500_response   = bundle$panel_500_response,
      methylation_response = bundle$methylation_response,
      clinical             = bundle$clinical,
      qc_filter            = qc_filter,
      baseline_visit       = baseline_visit,
      verbose              = verbose)
    if (is.list(ctdna_out)) {
      # v0.41.0: the platform handler still produces frames with
      # internal canonical names ("subject_id", "time_point", "dose",
      # etc.). Translate to the current ctdna_opts() column names so
      # the output matches what the rest of the package expects.
      handler_rename <- c(
        subject_id      = c_subj,
        time_point      = .o("time"),
        dose            = .o("dose"),
        ...
        methylTF        = .o("tf"),
        maxVAF          = .o("maxvaf"),
        best_pct_change = .o("best_change"),
        MR              = .o("mr"),
        gene            = .o("gene"),
        panel           = .o("panel"),
        mutation_status = .o("mutation"),
        alteration_type = .o("alteration"),
        vaf             = .o("vaf"))
      for (nm in names(ctdna_out)) {
        d <- ctdna_out[[nm]]
        if (!is.data.frame(d)) { out[[nm]] <- d; next }
        for (old_nm in names(handler_rename)) {
          new_nm <- handler_rename[[old_nm]]
          if (old_nm != new_nm && old_nm %in% names(d))
            names(d)[names(d) == old_nm] <- new_nm
        }
        out[[nm]] <- d
      }
    }
  } else if (verbose) {
    message(" - ctDNA: skipped (no ctDNA inputs in `sim`).")
  }

  # ---- IHC ----
  if (!is.null(bundle$ihc)) {
    if (verbose) message(" - IHC")
    out$ihc <- .prep_ihc(bundle$ihc, c_subj)
  } else if (verbose) {
    message(" - IHC: skipped (no `ihc` in `sim`).")
  }

  # ---- DNA-seq (accepts dnaseq or legacy wes) ----
  dnaseq_in <- if (!is.null(bundle$dnaseq)) bundle$dnaseq else bundle$wes
  if (!is.null(dnaseq_in)) {
    if (verbose) message(" - DNA-seq (tissue alteration calls)")
    out$dnaseq <- .prep_dnaseq(dnaseq_in, c_subj)
  } else if (verbose) {
    message(" - DNA-seq: skipped (no `dnaseq` or `wes` in `sim`).")
  }

  # ---- RNA-seq -> long-form expression ----
  if (!is.null(bundle$rnaseq)) {
    if (verbose) message(" - RNA-seq -> long-form expression (log2(TPM + 1))")
    out$expression <- .prep_expression(bundle$rnaseq, c_subj, c_gene, c_expr)
  } else if (verbose) {
    message(" - RNA-seq: skipped (no `rnaseq` in `sim`).")
  }

  # ---- TMB (derived from infinity_report) ----
  if (!is.null(bundle$infinity_report) &&
      "TMB_score" %in% names(bundle$infinity_report)) {
    if (verbose) message(" - TMB (extracted from infinity_report)")
    out$tmb <- .prep_tmb(bundle$infinity_report, c_subj)
  } else if (verbose && !is.null(bundle$infinity_report)) {
    message(" - TMB: skipped (no `TMB_score` in `infinity_report`).")
  }

  # ---- Cleanup (ctDNA / genomic only; cleanup is designed for those
  #      frames and applies a library-size QC filter that doesn't make
  #      sense for IHC / DNA-seq / TMB / expression) ----
  if (isTRUE(cleanup)) {
    ctdna_genomic <- intersect(c("ctdna","genomic_74","genomic_500"),
                                 names(out))
    for (nm in ctdna_genomic) {
      if (is.data.frame(out[[nm]]))
        out[[nm]] <- ctdna_cleanup(out[[nm]], quiet = !verbose)
    }
  }

  # ---- Filter scheme: already applied pre-dispatch on infinity_report
  #      above; the genomic_74 / genomic_500 frames inherit that
  #      filtering. Other modality frames (dnaseq) are also alteration
  #      frames with vendor column names — apply scheme to them too. ----
  if (!is.null(filter_scheme)) {
    if ("dnaseq" %in% names(out) && is.data.frame(out$dnaseq) &&
        any(c("Variant_type","Gene") %in% names(out$dnaseq))) {
      if (verbose) message(" - applying filter_scheme '", filter_scheme,
                            "' to dnaseq")
      out$dnaseq <- ctdna_filter_apply(out$dnaseq,
                                        filter_scheme = filter_scheme,
                                        verbose = verbose)
    }
  }

  # ---- Apply new_col_names overrides across every frame ----
  if (!is.null(new_col_names)) {
    if (!is.list(new_col_names) || is.null(names(new_col_names)) ||
        any(!nzchar(names(new_col_names))))
      stop("ctdna_prepare: `new_col_names` must be a NAMED list, e.g. ",
           "list(subject_id = \"Patient_ID\", gene = \"Gene_symbol\").",
           call. = FALSE)
    for (nm in names(out)) {
      d <- out[[nm]]
      if (!is.data.frame(d)) next
      for (old_nm in names(new_col_names)) {
        new_nm <- new_col_names[[old_nm]]
        if (!is.character(new_nm) || length(new_nm) != 1L)
          stop("ctdna_prepare: every value in `new_col_names` must be a ",
               "single string. Failed at: ", old_nm, call. = FALSE)
        if (old_nm %in% names(d)) names(d)[names(d) == old_nm] <- new_nm
      }
      out[[nm]] <- d
    }
  }
  ...
  # v0.43.0: ctdna_prepare() is pure format conversion now. The previous
  # end-of-function sanity check that errored on empty ctdna/genomic
  # frames has been removed -- empty outputs are valid (e.g. clinical-only
  # workflows). Use ctdna_qc_filter() afterwards to apply sample-level QC.

  out
}


#' Apply sample-level QC filtering to a prepared ctDNA bundle
#'
#' Operates on the output of \code{\link{ctdna_prepare}}. Uses the
#' \code{$samples} frame (populated automatically by the GH Infinity
#' platform handler) to decide which samples pass the user-supplied
#' QC thresholds, then filters the longitudinal \code{$ctdna} and
#' baseline \code{$genomic_74} / \code{$genomic_500} frames so only
#' passing samples and their patients are retained.
#'
#' \strong{v0.43.0}: \code{ctdna_prepare()} no longer applies any
#' sample-level filtering on its own; use this function explicitly
#' after \code{ctdna_prepare()} to apply QC.
#'
#' @param prep The output of \code{ctdna_prepare()}.
#' @param sample_status Character vector. Only samples whose
#'   \code{Sample_status} value matches one of these (case-insensitive)
#'   are kept. Default \code{NULL} = no Sample_status filter.
#' @param plasma_min Numeric. Minimum required \code{Plasma_ml_input}.
#'   Samples below this are dropped. Default \code{0} = no filter.
#' @param dna_ng_min Numeric. Minimum required \code{cfDNA_ng}.
#'   Samples below this are dropped. Default \code{0} = no filter.
#' @param verbose Logical. Print a summary of how many samples
#'   passed. Default \code{TRUE}.
#' @return A filtered version of \code{prep}. The \code{$samples}
#'   frame contains only passing samples; \code{$ctdna} is filtered
#'   to passing \code{(Patient_ID, Visit_name)} pairs; \code{$genomic_74}
#'   / \code{$genomic_500} are filtered to patients who have at least
#'   one passing sample.
#' @export
#' @examples
#' \dontrun{
...
#'                        clinical        = pan_adrs)
#'
#' # Keep PASS samples with >= 1 mL plasma input and >= 5 ng cfDNA:
#' prep <- ctdna_qc_filter(prep,
#'                          sample_status = c("PASS"),
#'                          plasma_min    = 1,
#'                          dna_ng_min    = 5)
#' }
ctdna_qc_filter <- function(prep,
                             sample_status = NULL,
                             plasma_min    = 0,
                             dna_ng_min    = 0,
                             verbose       = TRUE) {
  ...
  if (!is.list(prep))
    stop("ctdna_qc_filter: `prep` must be a list (output of ctdna_prepare).",
         call. = FALSE)
  if (is.null(prep$samples) || !is.data.frame(prep$samples) ||
      nrow(prep$samples) == 0L) {
    if (verbose)
      message("ctdna_qc_filter: no `samples` frame in prep; nothing to filter.")
    return(prep)
  }
  if (!is.numeric(plasma_min) || length(plasma_min) != 1L || plasma_min < 0)
    stop("ctdna_qc_filter: `plasma_min` must be a single non-negative number.",
         call. = FALSE)
  if (!is.numeric(dna_ng_min) || length(dna_ng_min) != 1L || dna_ng_min < 0)
    stop("ctdna_qc_filter: `dna_ng_min` must be a single non-negative number.",
         call. = FALSE)
  ...
  if (!is.null(sample_status) && length(sample_status) > 0L) {
    if (!"Sample_status" %in% names(s)) {
      warning("ctdna_qc_filter: `sample_status` filter requested but ",
              "`samples` has no Sample_status column; skipping that filter.",
              call. = FALSE)
    } else {
      keep <- tolower(trimws(as.character(s$Sample_status))) %in%
                tolower(as.character(sample_status))
      mask <- mask & keep
      drops$sample_status <- sum(!keep)
    }
  }
  if (plasma_min > 0) {
    if (!"Plasma_ml_input" %in% names(s)) {
      warning("ctdna_qc_filter: `plasma_min` filter requested but ",
              "`samples` has no Plasma_ml_input column; skipping that filter.",
              call. = FALSE)
    } else {
      pv   <- suppressWarnings(as.numeric(s$Plasma_ml_input))
      keep <- !is.na(pv) & pv >= plasma_min
      mask <- mask & keep
      drops$plasma_min <- sum(!keep)
    }
  }
  if (dna_ng_min > 0) {
    if (!"cfDNA_ng" %in% names(s)) {
      warning("ctdna_qc_filter: `dna_ng_min` filter requested but ",
              "`samples` has no cfDNA_ng column; skipping that filter.",
              call. = FALSE)
    } else {
      dn   <- suppressWarnings(as.numeric(s$cfDNA_ng))
      keep <- !is.na(dn) & dn >= dna_ng_min
      mask <- mask & keep
      drops$dna_ng_min <- sum(!keep)
    }
  }

  s_pass <- s[mask, , drop = FALSE]
  if (verbose) {
    message(sprintf("ctdna_qc_filter: %d of %d samples pass (%.1f%%)",
                    sum(mask), n0,
                    if (n0 > 0) 100 * sum(mask) / n0 else 0))
    if (length(drops) > 0L) {
      for (nm in names(drops))
        message(sprintf("  - dropped by %-15s: %d", nm, drops[[nm]]))
    }
  }

  # Build the set of passing (Patient_ID, Visit_name) keys, and the
  # set of patients with at least one passing sample.
  pass_keys     <- if ("Visit_name" %in% names(s_pass) && "Patient_ID" %in% names(s_pass))
                     paste(s_pass$Patient_ID, s_pass$Visit_name, sep = "|")
                   else character(0)
  pass_patients <- if ("Patient_ID" %in% names(s_pass))
                     unique(s_pass$Patient_ID) else character(0)

  # Filter $ctdna (longitudinal) by (Patient_ID, Visit_name) where possible
  if (!is.null(prep$ctdna) && is.data.frame(prep$ctdna) && nrow(prep$ctdna) > 0L) {
    cd <- prep$ctdna
    if (all(c("Patient_ID","Visit_name") %in% names(cd))) {
      ck <- paste(cd$Patient_ID, cd$Visit_name, sep = "|")
      prep$ctdna <- cd[ck %in% pass_keys, , drop = FALSE]
    } else if ("Patient_ID" %in% names(cd)) {
      prep$ctdna <- cd[cd$Patient_ID %in% pass_patients, , drop = FALSE]
    }
  }

  # Filter $genomic_74 / $genomic_500 by patient (these are per-(patient, gene))
  for (g_key in c("genomic_74","genomic_500","genomic")) {
    g <- prep[[g_key]]
    if (!is.null(g) && is.data.frame(g) && nrow(g) > 0L) {
      subj_col <- intersect(c("subject_id","Patient_ID","subject"), names(g))[1]
      if (!is.na(subj_col))
        prep[[g_key]] <- g[g[[subj_col]] %in% pass_patients, , drop = FALSE]
    }
  }
...
# ============================================================================
# Multi-file readers for orthogonal modalities (dnaseq / rnaseq)
# ----------------------------------------------------------------------------
...
#                 regex = "*/variant/annotated/abcetc/*.clinical.annotated.tsv")
#
# Glob-style pattern (dots literal, "*" is a wildcard). The reader walks the
# tree, reads every match, extracts Sample_ID from the FILENAME wildcard,
# canonicalises columns via ctdna_opts() keys, and returns a unified frame.
# ============================================================================

# ---- Column-name opts registry helpers -------------------------------------
# These read the canonical column name for each field from ctdna_opts(),
# giving users a stable customisation point via
#   ctdna_opts(dnaseq_gene_col = "GENE") etc.
.ctdna_dnaseq_defaults <- function() {
  list(
    ...
    dnaseq_qc_filter_col        = "Filter",           # optional
    dnaseq_min_vaf              = 0)
}
...
# Given a glob pattern like ".../*.clinical.annotated.tsv", return a regex
# that matches the same files. Glob wildcards: `*` -> `[^/]*`; `?` -> `[^/]`.
# Dots and other regex metachars are escaped as literals. Path separators
# stay as literal `/`.
.co_glob_to_regex <- function(pattern) {
  # Escape everything, then unescape wildcards.
...
# Extract Sample_ID from a filename given the filename portion of the glob
# pattern. Example: pattern = "*.clinical.annotated.tsv",
# filename = "sampleA.clinical.annotated.tsv" -> "sampleA".
# If pattern has multiple `*`, only the FIRST wildcard segment becomes the ID.
.co_sample_id_from_filename <- function(filename, pattern) {
  # Convert pattern to regex with one capture group for the FIRST `*`.
  # Escape everything except wildcards; replace first `*` with `(.*?)`,
  # remaining `*` with `.*`.
  esc <- gsub("([][{}().+^$|\\\\])", "\\\\\\1", pattern, perl = TRUE)
  # Replace only first * with capture group
  esc <- sub("\\*", "(.*?)", esc)
  esc <- gsub("\\*", ".*", esc)
...
# Robust delimited reader: try TSV first, then CSV; fall back to auto.
.co_read_delim <- function(path) {
  # Read as text with base to avoid dep on data.table/readr
  # Try TSV
  txt <- tryCatch(
...
# ---- Main reader: walk tree, read every match, canonicalise, concatenate --
#' Read a directory tree of per-sample delimited files into one long frame
#'
#' Walks `loc` looking for files matching `regex` (glob pattern), reads each,
#' extracts Sample_ID from the filename wildcard, canonicalises columns via
#' ctdna_opts() keys, and concatenates. Used internally by ctdna_prepare()
#' when the user supplies `dnaseq = list(loc = ..., regex = ...)` or the
#' equivalent for rnaseq.
#'
#' @param loc Directory root (character path).
#' @param regex Glob pattern relative to `loc`. Example:
#'   "*/variant/annotated/*.clinical.annotated.tsv"
#'   - Dots are literal
#'   - `*` matches any character except path separator
#'   - The FIRST `*` in the FILENAME (last path segment) becomes Sample_ID
#' @param modality "dnaseq" (default) or "rnaseq" -- selects which opts keys
#'   drive the column canonicalisation.
#' @param verbose Show a progress bar and per-file status. Default `TRUE`.
#'
#' @return A data.frame with (at minimum) Sample_ID, Patient_ID, Gene,
#'   Variant_type. Additional canonical columns are added per opts settings.
#'   Rows failing the QC filter (if `dnaseq_qc_filter_col` exists) or below
#'   `dnaseq_min_vaf` are dropped.
#'
#' @keywords internal
.ctdna_read_delimited_tree <- function(loc, regex,
  ...
  # Expand the glob relative to loc
  ...
  # Filename portion of the pattern for Sample_ID extraction
  fname_pat <- basename(regex)

  # Which opts keys we need
  if (modality == "dnaseq") {
    o <- .ctdna_dnaseq_defaults()
    for (k in names(o)) o[[k]] <- ctdna_opts(k, default = o[[k]])
  ...
  for (i in seq_along(files)) {
    ...
    sid <- .co_sample_id_from_filename(basename(f), fname_pat)
    if (is.na(sid) || !nzchar(sid))
      sid <- tools::file_path_sans_ext(basename(f))

    df <- tryCatch(.co_read_delim(f), error = function(e) {
      errors <<- c(errors, sprintf("%s: %s", f, conditionMessage(e)))
      NULL
    })
    if (is.null(df) || !nrow(df)) next

    # Add Sample_ID + Patient_ID (same for tissue: one sample per patient)
    df$Sample_ID  <- sid
    df$Patient_ID <- sid

    # Canonical column mapping — rename user's columns to package canonical
    rn <- function(from, to) {
    ...
    # QC filter (only if the column is present)
    ...
    # VAF filter (dnaseq only)
    if (modality == "dnaseq" && "Allelic_Fraction" %in% names(df) && min_vaf > 0) {
      vaf <- suppressWarnings(as.numeric(df$Allelic_Fraction))
      # accept 0-1 or percent; normalise to 0-1
  ...
  # Concatenate. Frames may have different columns per site -> use bind
  # style union.
  out <- .co_rbind_union(frames)
...
.co_rbind_union <- function(frames) {
