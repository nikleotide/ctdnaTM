...
#                   best_pct_change, MR)
#   $genomic_74  : baseline per-(subject x gene) calls
...
.gh_build_ctdna_long <- function(tf_change,
  ...
  # Long-format: one row per (Patient x Visit). Both legs of the pair
...
.gh_ctdna_from_infinity_report <- function(infinity_report, clinical = NULL,
  ...
  if (verbose)
    message(sprintf("  built $ctdna from infinity_report: %d samples x %d cols",
                    nrow(ct), ncol(ct)))
  ct
}
...
# One row per (subject x gene) for the panel's gene list. Detected
...
.gh_build_genomic <- function(infinity_report,
  ...
  # Enumerate wt rows for (Patient x Gene) combos that weren't detected.
  ...
  rownames(out) <- NULL
  if (verbose)
    message(sprintf("  %s: %d detected + %d wt = %d rows x %d cols",
...
# NOTE: the ctdna_prepare() entry point is defined and documented in
# prepare.R. A stale duplicate of its docstring used to sit here; it has
# been removed so the block below documents its intended target
# (.ctdna_column_aliases) rather than being co-located with ctdna_prepare's
# args (which caused an R CMD check Rd \usage WARNING).
...
#' Canonical column-name aliases (internal helper)
#'
#' Zero-argument helper that returns the master table of accepted
#' vendor / user column names for every canonical ctdnaTM column. Used
#' by the platform handlers and by \code{\link{ctdna_prepare}} to
#' rename incoming columns to the canonical set defined by
#' \code{\link{ctdna_opts}}.
#'
#' @return A named list. Names are the canonical column names
#'   (\code{Patient_ID}, \code{Sample_ID}, \code{Visit_name}, ...);
#'   each element is a character vector of accepted aliases whose
