...
.defaults <- list(
  ...
  vaf           = "MAF",                   # v0.41.0: new — vendor uses MAF for variant allele fraction (percent)
  ...
  # Sample-level QC is applied INSIDE ctdna_filter_apply() in all three
  ...
  # in scheme_HRR path B + scheme_TSG rare-pathogenic-synonymous carve-
...
.ctdna_method_registry <- function() list(
  tf = list(
    methyl  = list(value_col = "methylTF",
                    label = "Methylation TF",
                    unit = "fraction", floor = 0.001,
                    display_unit = "percent"),
    maxvaf  = list(value_col = "maxVAF",
                    label = "Max VAF",
                    unit = "fraction", floor = 0.001,
                    display_unit = "percent"),
    meanvaf = list(value_col = "meanVAF",
                    label = "Mean VAF",
                    unit = "fraction", floor = 0.001,
                    display_unit = "percent"),
...
.ctdna_filter_registry_defaults <- function() list(
  ...
  #                     (step 6 of ctdna_filter_apply hierarchy). Pure QC.
...
#' \code{\link{ctdna_filter_apply}} (or pass via \code{filter_set} to
...
# every enabled entry. Used by ctdna_filter_apply(); not exported
# as a user-facing function (ctdna_filter_apply is the entry point).
#
...
#    template, built-in v1.0.0 scheme like scheme_TSG/scheme_HRR, or
...
#'   block's rules; \code{\link{ctdna_filter_apply}} to apply blocks.
#' @examples
...
ctdna_filter_schemes_list <- function(category = c("all","indication_gene_set",
  ...
  # v1.0.0 built-in schemes (scheme_HRR / scheme_TSG)
...
ctdna_filter_scheme_show <- function(name) {
  ...
  # v1.0.0: built-in schemes (scheme_HRR / scheme_TSG) and user-registered
...
#   3. gene-set schemes  (claim their rows, includes scheme_HRR, scheme_TSG)
...
# Sentinel value used to detect "argument not supplied". Distinct from NULL,
# which the user can pass explicitly to mean "no filter at all".
.ctdna_filter_scheme_default_sentinel <- function()
  structure(list(), class = ".ctdna_use_session_default")

# Deprecation warning cache (one-time-per-session per alias)
...
.ctdna_check_cancertypes <- function(df) {
  ...
  if (length(unknown) > 0) {
    warning(
      "\n",
      "  *** ctdna_filter_apply: UNRECOGNIZED CANCERTYPE VALUES ***\n",
      "  Found in column '", col, "': ",
...
.ctdna_resolve_schemes <- function(filter_scheme) {
  ...
  for (nm in filter_scheme) {
    if (nm %in% names(builtin)) {
      ...
      # Wrap a thin pseudo-scheme so ctdna_filter_apply() can still
...
#'     built-in \code{scheme_HRR} and \code{scheme_TSG}
...
#'       \code{c("scheme_HRR","scheme_TSG")} or \code{c("HRR","NSCLC")}.
...
#'   \code{\link{scheme_HRR}}, \code{\link{scheme_TSG}},
#'   \code{\link{ctdna_filter_schemes_list}}.
#' @examples
#' \dontrun{
#' # Apply built-in HRR scheme alone
#' df_hrr <- ctdna_filter_apply(df, filter_scheme = "scheme_HRR")
#'
#' # Multiple schemes — each claims its rows, unclaimed go to general
#' df_combined <- ctdna_filter_apply(df,
#'   filter_scheme = c("scheme_HRR","scheme_TSG"))
#'
...
#'   name = "my_egfr_kras")
#' df_my <- ctdna_filter_apply(df, filter_scheme = my)
#' }
#' @export
ctdna_filter_apply <- function(df,
                                 filter_scheme = .ctdna_filter_scheme_default_sentinel(),
                                 gene_col = NULL,
  ...
  # --- Detect: caller explicitly passed filter_scheme (could be NULL) ----
  use_session_default <- inherits(filter_scheme, ".ctdna_use_session_default")
  ...
  if (qc_active && n_pre_qc > 0L) {
    ...
    rownames(df) <- NULL
    if (qc_dropped > 0L)
      msg(sprintf("ctdna_filter_apply: QC dropped %d / %d row(s).",
                   qc_dropped, n_pre_qc))
  }
  ...
  # ----------------------------------------------------------------------
  if (!use_session_default && is.null(filter_scheme)) {
    msg("ctdna_filter_apply: filter_scheme = NULL -> QC only, no variant filtering.")
  ...
  # --- Resolve session default + deprecation aliases --------------------
  if (use_session_default) {
    filter_scheme <- .o("default_scheme") %||% "basic"
  }
  if (is.character(filter_scheme) && length(filter_scheme) == 1L &&
      filter_scheme == "none") {
    msg("ctdna_filter_apply: filter_scheme = 'none' -> no filtering.")
    attr(df, "ctdna_filter_used") <- list(mode = "none", scheme = NULL,
                                            n_in = nrow(df), n_out = nrow(df))
    return(df)
  }
  if (is.character(filter_scheme) && length(filter_scheme) == 1L &&
      filter_scheme %in% c("general","default")) {
    .ctdna_warn_once(filter_scheme, sprintf(
      "filter_scheme = '%s' is DEPRECATED; use 'basic' instead.",
      filter_scheme))
    filter_scheme <- "basic"
  }
  ...
  if (is.character(filter_scheme) && length(filter_scheme) == 1L &&
      filter_scheme == "basic") {
    msg("ctdna_filter_apply: BASIC mode (scheme_basic on all rows).")
  ...
  msg(sprintf("ctdna_filter_apply: %d scheme(s): %s",
...
#'
#' Tile plot of patients (columns) x genes (rows) coloured by the eleven
#' canonical alteration classes (\code{Focal_Amp}, \code{Amp},
#' \code{Homozygous_Del}, \code{LOH}, \code{Missense}, \code{Truncating},
#' \code{InFrame}, \code{Promoter}, \code{LGR}, \code{Fusion}, \code{Other}).
#' When ComplexHeatmap is installed, \code{ComplexHeatmap::oncoPrint()}
#' renders the output with a top per-patient burden bar, a right-margin
#' per-gene frequency bar, optional left-annotation gene-set strips, stacked
#' annotation tracks, gene-set row splits, and a merged legend. Otherwise
#' a ggplot tile-plot fallback is used (full feature parity except for the
#' top burden bar).
#'
...
#' \code{gene_sets} can be:
#' \itemize{
#'   \item \code{NULL} — all genes in \code{df} under one heading.
#'   \item A character scalar or vector of \emph{built-in} gene-set names:
#'     \code{"HRR14"}, \code{"TSG"}, \code{"RTK"}, \code{"Cell_Cycle"},
#'     \code{"TP53_pathway"}, \code{"MMR"}, \code{"PI3K"}.
#'   \item A NAMED list of gene-symbol vectors (inline definition), e.g.
#'     \code{list(My_drivers = c("EGFR","KRAS"))}.
#' }
#'
...
#'     frequency, regardless of set membership. The left annotation shows
#'     which set(s) each gene belongs to (one column per set; the same gene
#'     can light up in multiple columns).
#'   \item \code{"within_set"} — sort genes by frequency WITHIN each set,
#'     with a horizontal gap separating sets. The left annotation is omitted
#'     (row-split bands already convey set membership).
#'   \item \code{"none"} — keep gene order as given in \code{gene_sets}.
#'     Same left-annotation behaviour as \code{"global"}.
#' }
#' For backwards compatibility \code{TRUE} maps to \code{"global"} and
#' \code{FALSE} to \code{"none"}.
#'
#' @section Wrap into multi-panel layouts:
#' \code{wrap} splits the cohort into one panel per level of a column (in
#' \code{patient_data} or \code{df}). Panels share one cumulative legend
#' that lists every value of each annotation across the whole cohort, but
#' each panel's per-gene prevalence bar is computed independently. The
#' column can be referenced by its canonical name or by its display label
#' in \code{annotation_labels}.
#'
#' @section Annotation labels:
#' \code{annotation_labels} is a named character vector
#' (e.g. \code{c(Response_Subcategory = "Response", Clin_Indication = "Indication")})
#' that renames both the annotation strip on the left of each track AND the
#' corresponding legend title. \code{group_patients_by} and \code{wrap} will
#' accept either the canonical column name or any of the display labels
#' defined here.
#'
#' @section RECIST scheme:
#' \code{scheme} controls how RECIST-flavoured columns (auto-detected by
#' name or value content) are collapsed via \code{\link{ctdna_stratify_recist}}:
#' \itemize{
#'   \item \code{"two"} — R / NR
#'   \item \code{"three"} — CR+PR / SD / PD+NE+NA  (default)
#'   \item \code{"four"} — CR+PR / SD / PD / NE+NA
#'   \item \code{"four_alt"} — CR / PR / SD / PD+NE+NA
#'   \item \code{"five"} — CR / PR / SD / PD / NE+NA
#'   \item \code{"six"} — CR / PR / SD / PD / NE / NA
#' }
#' Schemes that distinguish CR from PR (\code{"four_alt"}, \code{"five"},
#' \code{"six"}) require a column with the raw CR/PR distinction. The
#' canonical \code{Response_Subcategory} produced by
#' \code{\link{ctdna_make_patient_data}} is pre-collapsed to \code{CR/PR}
#' and so cannot be split further; for those schemes, supply a raw RECIST
#' column (e.g. the BCR/AVALC value from ADRS).
#'
#' @section Engine:
#' \code{engine} selects the backend:
#' \itemize{
#'   \item \code{"auto"} (default) — use ComplexHeatmap if available,
#'     otherwise the ggplot fallback.
#'   \item \code{"complexheatmap"} — force ComplexHeatmap; error with
#'     install instructions if the package is missing.
#'   \item \code{"ggplot"} — force the ggplot fallback. With
#'     \code{wrap} or \code{top_annotations} the assembled layout uses
#'     \pkg{patchwork} if installed.
#' }
#'
#' @section Filtering shortcut:
#' \code{filter_scheme} (optional) is applied as a pre-filter via
#' \code{\link{ctdna_filter_apply}}. Identical to
#' \code{ctdna_filter_apply(df, filter_scheme = ...)} before the call.
#'
#' @param df Alteration data frame (vendor or canonical schema). Required
#'   columns: \code{Gene}, \code{Variant_type}, \code{Molecular_consequence},
#'   \code{CNV_type}, plus a patient ID column.
#' @param gene_sets See "Gene-set API". Default \code{NULL}.
#' @param patient_data Optional one-row-per-patient frame with the patient
#'   ID and any annotation columns. Required if you use
#'   \code{top_annotations}, \code{wrap}, \code{group_patients_by}, or
#'   \code{cancer_type}. Accepts either a plain data frame or a
#'   \code{ctdna_patient_data} object from
#'   \code{\link{ctdna_make_patient_data}}.
#' @param top_annotations Character vector of column names in
#'   \code{patient_data} to display as stacked tracks above the oncoprint.
#' @param annotation_labels Named character vector mapping
#'   \code{<column name>} -> \code{<display label>}. Used for strip
#'   labels, legend titles, and as an alias source for
#'   \code{group_patients_by} / \code{wrap}.
#' @param cancer_type Optional character vector of indication values
#'   (e.g. \code{"NSCLC"}). Restricts the cohort to patients with that
#'   value in \code{cancer_type_col} of \code{patient_data}.
#' @param cancer_type_col Column in \code{patient_data} holding the
#'   indication. Default \code{"Cancer_Type"}.
#' @param visit Optional character vector restricting \code{df} to rows
#'   where \code{visit_col} matches. Typical: \code{visit = "Cycle 1 Day 1"}
#'   for a baseline oncoprint. Default \code{NULL}.
#' @param visit_col Column in \code{df} holding the visit label.
#'   Default \code{"Visit_name"}.
#' @param restrict_to_clinical If \code{TRUE} (default), drop genomic
#'   patients with no row in \code{patient_data} (root cause of mysterious
#'   grey annotation columns).
#' @param scheme RECIST collapse for any RECIST-flavoured column in use.
#'   See "RECIST scheme". Default \code{"three"}.
#' @param show_freq_bar Show right-side per-gene frequency barplot
#'   (per-panel when \code{wrap} is set).
#' @param show_patient_names Display each patient's ID under their column.
#'   Default \code{FALSE}.
#' @param show_all_patients Controls which patients appear as columns:
#' \itemize{
#'   \item \code{FALSE} (default) — drop patients with zero displayed
#'     alterations.
#'   \item \code{TRUE} — keep every patient in the resolved cohort,
#'     including those with zero alterations (rendered as empty grey
#'     columns). The cohort is taken from \code{patient_data} when
#'     provided, else from the unique patient IDs in \code{df}.
#'   \item \strong{Character vector of patient IDs} — use this vector
#'     directly as the column space, regardless of what's in \code{df}
#'     or \code{patient_data}. Patients in the vector that have no
#'     alterations in \code{df} render as empty columns; patients
#'     present in \code{df} but absent from the vector are dropped.
#'     This is the right option when filtering has already removed
#'     unaltered patients from \code{df}: pass the patient IDs from
#'     the \emph{pre-filter} frame, e.g.
#'     \code{show_all_patients = unique(unfiltered_df$Patient_ID)}.
#' }
#' Default \code{FALSE}.
#' @param group_patients_by Optional column name (or display label) in
#'   \code{patient_data} used to sort patients. Within each group,
#'   columns are sorted by mutation burden descending. RECIST-flavoured
#'   sort keys follow the canonical RECIST order; factor sort keys
#'   follow factor levels.
#' @param wrap Optional column name (or display label). Splits the
#'   cohort into one panel per level of this column. See "Wrap".
#' @param filter_scheme Optional. If set, the function calls
#'   \code{\link{ctdna_filter_apply}(df, filter_scheme = filter_scheme)}
#'   as a pre-filter before plotting.
#' @param patient_col,gene_col,variant_col Column overrides.
#' @param sort_genes \code{"global"} / \code{"within_set"} / \code{"none"}.
#'   See "Gene sorting". \code{TRUE}/\code{FALSE} also accepted.
#' @param alterations Optional character vector restricting which
#'   alteration classes are shown. Names must come from
#'   \code{\link{ctdna_alteration_types}()}: \code{"Focal_Amp"},
#'   \code{"Amp"}, \code{"Homozygous_Del"}, \code{"LOH"},
#'   \code{"Missense"}, \code{"Truncating"}, \code{"InFrame"},
#'   \code{"Promoter"}, \code{"LGR"}, \code{"Fusion"}, \code{"Other"}.
#'   \code{NULL} (default) shows all 11. Case-sensitive.
#' @param ... Individual alteration flags as a memorize-free alternative
#'   to \code{alterations =}. Pass any subset of the 11 canonical class
#'   names with a \code{TRUE} / \code{FALSE} value:
#'   \itemize{
#'     \item \strong{Whitelist} (only \code{TRUE} flags): show only
#'       those classes. \code{Focal_Amp = TRUE, Missense = TRUE} shows
#'       only Focal_Amp and Missense.
#'     \item \strong{Blacklist} (only \code{FALSE} flags): show every
#'       class \emph{except} those.
#'       \code{ctdna_plot_oncoprint(df, Focal_Amp = FALSE)} shows the
#'       other ten classes. Useful when you want to drop one or two
#'       noisy categories without naming the other nine.
#'     \item Mixed \code{TRUE} / \code{FALSE} flags: positives win
#'       (warning emitted).
#'     \item Unknown name in \code{...}: hard error listing the 11
#'       canonical names.
#'   }
#'   If both \code{alterations =} and \code{...} flags are supplied,
#'   \code{alterations =} wins (warning emitted).
#' @param title,subtitle,caption Display labels (passed to both engines).
...
#'
#' # NSCLC-only with one annotation track
#' ctdna_plot_oncoprint(sim$infinity_report,
...
#'
#' # Multiple gene-sets, separated horizontal bands, sorted within set
#' ctdna_plot_oncoprint(sim$infinity_report,
#'                       gene_sets = c("HRR14","TSG"),
#'                       sort_genes = "within_set")
#'
#' # Restrict to a small set of alteration classes (canonical vector form)
#' ctdna_plot_oncoprint(sim$infinity_report,
#'                       alterations = c("Missense","Truncating"))
#'
#' # Drop one noisy class without naming the other ten (blacklist form)
#' ctdna_plot_oncoprint(sim$infinity_report, Other = FALSE)
#'
#' # Whitelist via individual flags (no need to remember the canonical names)
#' ctdna_plot_oncoprint(sim$infinity_report,
#'                       Focal_Amp = TRUE, Missense = TRUE)
#'
#' # Discover the canonical names
#' ctdna_alteration_types()
#'
#' # Wrap by indication; cumulative legend across panels
#' ctdna_plot_oncoprint(sim$infinity_report,
#'                       gene_sets       = "HRR14",
#'                       patient_data    = sim$clinical,
#'                       top_annotations = "RECIST",
...
ctdna_plot_oncoprint <- function(df,
                                  ...
                                  restrict_to_clinical = TRUE,
                                  scheme          = c("three","two","four",
                                                       "four_alt","five","six"),
                                  show_freq_bar   = TRUE,
  ...
  # v0.42.0: alteration-type filtering.
  #   alterations = NULL              -> show every type (default)
  #   alterations = c("Focal_Amp",..) -> show only those
  #   passed as flags via ...:
  #     Focal_Amp = TRUE  / Missense = TRUE   (positives only) -> whitelist
  #     Focal_Amp = FALSE / Other = FALSE     (negatives only) -> blacklist
  #                                                              (all except these)
  #     mixed TRUE/FALSE flags                                  -> use positives
  #                                                              (warn once)
  #   alterations supplied AND any flags supplied               -> use alterations,
  #                                                              warn once
  # Anything else in `...` (unknown name) errors with the canonical list.
  keep_alterations <- .resolve_oncoprint_alterations(alterations, list(...))

  # ---- Optional pre-filter via the three-level filter system ----
  if (!is.null(filter_scheme))
    df <- ctdna_filter_apply(df, filter_scheme = filter_scheme)

  # ---- patient_data may arrive as a ctdna_patient_data object ----
  if (!is.null(patient_data))
    patient_data <- .resolve_patient_data(patient_data)

  # ---- Column auto-detection on df ----
  cols <- .oncoprint_autodetect_cols(df, patient_col, gene_col, variant_col)
  patient_col <- cols$patient_col
  gene_col    <- cols$gene_col
  variant_col <- cols$variant_col

  # ---- Drop unusable rows ----
  df <- df[!is.na(df[[gene_col]]) & nzchar(df[[gene_col]]), , drop = FALSE]
  df <- df[!is.na(df[[patient_col]]) & nzchar(df[[patient_col]]), , drop = FALSE]

  # ---- Visit filter ----
  if (!is.null(visit)) {
    if (!visit_col %in% names(df))
      stop(sprintf(
        "ctdna_plot_oncoprint: `visit` was set but column '%s' is not in df. ",
        visit_col),
        "Set `visit_col` to the right column or pass `visit = NULL`.",
        call. = FALSE)
    n_before <- nrow(df)
    df <- df[!is.na(df[[visit_col]]) & df[[visit_col]] %in% visit, ,
              drop = FALSE]
    if (nrow(df) == 0L)
      stop(sprintf(
        "ctdna_plot_oncoprint: no rows left after filtering to visit = c(%s).",
        paste(shQuote(visit), collapse = ", ")), call. = FALSE)
    if (nrow(df) < n_before)
      message(sprintf(
        "ctdna_plot_oncoprint: visit filter kept %d of %d rows.",
        nrow(df), n_before))
  }

  # ---- Cancer-type restriction ----
  if (!is.null(cancer_type)) {
    if (is.null(patient_data))
      stop("ctdna_plot_oncoprint: `cancer_type` requires `patient_data`.",
           call. = FALSE)
    # Fuzzy-spell the cancer_type_col so callers can use either canonical
    # name without thinking about it.
    if (!cancer_type_col %in% names(patient_data)) {
      alt <- if (cancer_type_col == "Cancer_Type") "Cancertype"
              else if (cancer_type_col == "Cancertype") "Cancer_Type"
              else NULL
      if (!is.null(alt) && alt %in% names(patient_data)) cancer_type_col <- alt
      else stop(sprintf(
        "ctdna_plot_oncoprint: `cancer_type_col = '%s'` not in patient_data.",
        cancer_type_col), call. = FALSE)
    }
    keep_patients <- as.character(patient_data[[cancer_type_col]]) %in% cancer_type
    if (!any(keep_patients))
      stop(sprintf(
        "ctdna_plot_oncoprint: no patients in patient_data have %s in c(%s).",
        cancer_type_col,
        paste(shQuote(cancer_type), collapse = ", ")), call. = FALSE)
    pid_col_pd <- intersect(c("Patient_ID","SUBJID","USUBJID","subject_id"),
                              names(patient_data))[1]
    if (is.na(pid_col_pd))
      stop("ctdna_plot_oncoprint: patient_data has no recognizable ID column.",
            call. = FALSE)
    keep_ids <- .norm_pid(patient_data[[pid_col_pd]][keep_patients])
    df <- df[.norm_pid(df[[patient_col]]) %in% keep_ids, , drop = FALSE]
    patient_data <- patient_data[keep_patients, , drop = FALSE]
    if (nrow(df) == 0L)
      stop("ctdna_plot_oncoprint: no df rows match patients with ",
            cancer_type_col, " in c(",
            paste(shQuote(cancer_type), collapse = ", "), ").",
            call. = FALSE)
    if (!is.null(title) && identical(title, "Oncoprint"))
      title <- sprintf("Oncoprint (%s)", paste(cancer_type, collapse = ", "))
  }

  # ---- restrict_to_clinical: drop genomic-only patients ----
  if (isTRUE(restrict_to_clinical) && !is.null(patient_data)) {
    pid_col_pd <- intersect(c("Patient_ID","SUBJID","USUBJID","subject_id"),
                              names(patient_data))[1]
    if (!is.na(pid_col_pd)) {
      clin_ids <- .norm_pid(patient_data[[pid_col_pd]])
      df <- df[.norm_pid(df[[patient_col]]) %in% clin_ids, , drop = FALSE]
    }
  }

  # ---- show_all_patients: validate + extract explicit cohort if given ----
  # Accepts FALSE, TRUE, or a character vector of patient IDs. A character
  # vector OVERRIDES cohort derivation from df/patient_data — use it when
  # filtering has already removed unaltered patients from the input.
  explicit_cohort <- NULL
  if (is.character(show_all_patients)) {
    if (!length(show_all_patients))
      stop("ctdna_plot_oncoprint: `show_all_patients` was a zero-length ",
           "character vector. Pass TRUE/FALSE, or a non-empty character ",
           "vector of patient IDs.", call. = FALSE)
    explicit_cohort <- unique(as.character(show_all_patients))
    explicit_cohort <- explicit_cohort[!is.na(explicit_cohort) &
                                         nzchar(explicit_cohort)]
    keep_zero_burden_cols <- TRUE
  } else if (is.logical(show_all_patients) &&
              length(show_all_patients) == 1L &&
              !is.na(show_all_patients)) {
    keep_zero_burden_cols <- isTRUE(show_all_patients)
    # When TRUE but no explicit cohort and the inferred cohort can't include
    # unaltered patients (no patient_data, or patient_data appears to be the
    # alteration frame), let the user know the canonical fix.
    if (isTRUE(show_all_patients)) {
      pid_col_pd <- if (!is.null(patient_data))
        intersect(c("Patient_ID","SUBJID","USUBJID","subject_id"),
                  names(patient_data))[1] else NA_character_
      looks_like_per_patient <- !is.null(patient_data) && !is.na(pid_col_pd) &&
        length(unique(as.character(patient_data[[pid_col_pd]]))) >=
        0.5 * nrow(patient_data)
      if (!looks_like_per_patient) {
        message("ctdna_plot_oncoprint: `show_all_patients = TRUE` will only ",
                "show patients that have at least one row in the input. To ",
                "include patients that were removed by filtering (no surviving ",
                "alterations), pass their IDs directly: ",
                "`show_all_patients = unique(unfiltered_df$Patient_ID)`.")
      }
    }
  } else {
    stop("ctdna_plot_oncoprint: `show_all_patients` must be TRUE, FALSE, or ",
         "a character vector of patient IDs.", call. = FALSE)
  }

  # ---- Default top_annotation ----
  if (is.null(top_annotations) && !is.null(patient_data) &&
      "Response_Subcategory" %in% names(patient_data))
    top_annotations <- "Response_Subcategory"

  # ---- Resolve aliases for group_patients_by / wrap (label-aware) ----
  group_patients_by <- .oncoprint_resolve_alias(group_patients_by,
                                                  "group_patients_by",
                                                  patient_data, df,
                                                  annotation_labels)
  wrap              <- .oncoprint_resolve_alias(wrap, "wrap",
                                                  patient_data, df,
                                                  annotation_labels)

  # ---- Gene sets ----
  gs <- .oncoprint_resolve_gene_sets(df, gene_sets, gene_col)
  resolved   <- gs$resolved
  gene_union <- gs$gene_union

  # ---- Restrict df to gene union and classify ----
  df <- df[df[[gene_col]] %in% gene_union, , drop = FALSE]
  if (nrow(df) == 0L)
    stop("ctdna_plot_oncoprint: no alteration rows remain after gene_set ",
         "restriction — nothing to plot.", call. = FALSE)
  df <- .oncoprint_classify(df)

  # v0.42.0: drop rows whose classified alteration_class isn't in the
  # user's keep_alterations set (resolved at the top of this function).
  # `alteration_class` is the column the classifier writes.
  if (!setequal(keep_alterations, ctdna_alteration_types())) {
    n_before <- nrow(df)
    df <- df[df$Alteration_class %in% keep_alterations, , drop = FALSE]
    if (nrow(df) == 0L)
      stop("ctdna_plot_oncoprint: no rows remain after restricting to ",
           "alteration types [", paste(keep_alterations, collapse = ", "),
           "].", call. = FALSE)
  }

  # ---- Apply RECIST scheme to any RECIST column in use ----
  sa <- .oncoprint_apply_scheme(patient_data, top_annotations,
                                  group_patients_by, scheme)
  patient_data    <- sa$patient_data
  recist_cols_used <- sa$recist_cols_used

  # ---- Wrap groups ----
  pd_id_col <- if (!is.null(patient_data))
    intersect(c("Patient_ID","SUBJID","USUBJID","subject_id"),
              names(patient_data))[1] else NA_character_
  wrap_info <- .oncoprint_resolve_wrap(wrap, patient_data, pd_id_col, df,
                                        patient_col)

  # ---- Build global matrix + ordering ----
  bm <- .oncoprint_build_matrix(df, patient_data, pd_id_col, patient_col,
                                  gene_col, variant_col, gene_union,
                                  resolved, sort_genes,
                                  explicit_cohort = explicit_cohort)
  global_mat    <- bm$global_mat
  ordered_genes <- bm$ordered_genes
  row_split     <- bm$row_split
  global_cohort <- bm$global_cohort

  # ---- Build cumulative legend state (covers every value across cohort) ----
  ls <- .oncoprint_build_legend_state(top_annotations, patient_data,
                                        recist_cols_used)
  ann_col_global <- ls$col
  ann_at_global  <- ls$at

  global_classes <- intersect(names(.oncoprint_alt_palette),
                              unique(df$Alteration_class))
  col_vec <- .oncoprint_alt_palette[global_classes]

  # ---- Engine dispatch ----
  use_ch <- engine == "complexheatmap" ||
            (engine == "auto" &&
              requireNamespace("ComplexHeatmap", quietly = TRUE))

  if (use_ch) {
    alter_fun <- .oncoprint_alter_fun(global_classes)
    # v0.39.1: build the unified gene-set legend once (covers all panels).
    gs_la_info <- .oncoprint_left_anno(ordered_genes, resolved, sort_genes)
    gs_legend  <- if (!is.null(gs_la_info)) gs_la_info$legend else NULL

    build_panel <- function(panel_pids, panel_title, is_first, is_last)
      .oncoprint_panel_ch(global_mat, ordered_genes, row_split,
                            panel_pids, panel_title, is_first, is_last,
    ...
    if (is.null(wrap_info)) {
      ht <- build_panel(global_cohort, title, TRUE, TRUE)
      if (is.null(ht)) stop("ctdna_plot_oncoprint: no patients to plot.",
                              call. = FALSE)
      return(structure(list(heatmap = ht, engine = "ComplexHeatmap",
                              legend_side = legend_position,
                              gene_set_legend = gs_legend,
    ...
    for (lv in wrap_info$levels) {
      h_probe <- build_panel(wrap_info$pat_map[[lv]], lv, TRUE, TRUE)
      if (!is.null(h_probe)) surviving <- c(surviving, lv)
    }
    if (length(surviving) == 0L)
      stop("ctdna_plot_oncoprint: no wrap level has any data to plot.",
           call. = FALSE)
    ht_list <- vector("list", length(surviving))
    for (i in seq_along(surviving))
      ht_list[[i]] <- build_panel(wrap_info$pat_map[[surviving[i]]],
                                    surviving[i],
                                    is_first = i == 1L,
                                    is_last  = i == length(surviving))
    combined <- Reduce(`+`, ht_list)
    return(structure(list(heatmap = combined, engine = "ComplexHeatmap",
                            legend_side = legend_position,
                            gene_set_legend = gs_legend,
                            subtitle = subtitle, caption = caption,
                            title = title),
  ...
  if (engine == "complexheatmap")
    stop("ctdna_plot_oncoprint: engine = 'complexheatmap' requires the ",
          "ComplexHeatmap package. Install with:\n",
          "  install.packages('BiocManager'); BiocManager::install('ComplexHeatmap')",
          call. = FALSE)

  build_panel_gg <- function(panel_pids, panel_title, is_first, is_last)
    .oncoprint_panel_gg(global_mat, ordered_genes, row_split,
                         panel_pids, panel_title, is_first, is_last,
                         ...
                         show_freq_bar,
                         global_classes)
  panels <- if (is.null(wrap_info)) {
    p <- build_panel_gg(global_cohort, title, TRUE, TRUE)
    if (is.null(p)) stop("ctdna_plot_oncoprint: no patients to plot.",
                          call. = FALSE)
    list(p)
  } else {
    out <- list()
    ...
    for (lv in wrap_info$levels) {
      p <- build_panel_gg(wrap_info$pat_map[[lv]], lv, TRUE, TRUE)
      if (!is.null(p)) { out[[length(out)+1L]] <- p; levels_seen <- c(levels_seen, lv) }
    }
    if (!length(out))
      stop("ctdna_plot_oncoprint: no wrap level has any data to plot.",
           call. = FALSE)
    # Reset is_first / is_last to reflect surviving panel positions.
    for (i in seq_along(out)) {
      out[[i]] <- build_panel_gg(wrap_info$pat_map[[levels_seen[i]]],
                                  levels_seen[i],
                                  is_first = i == 1L,
                                  is_last  = i == length(out))
    }
    out
  }
  assembled <- .oncoprint_assemble_gg(panels, title, subtitle, caption,
                                       legend_position)
  structure(list(plot = assembled, engine = "ggplot2",
                  subtitle = subtitle, caption = caption,
                  title = title),
            class = c("ctdna_oncoprint", "list"))
}
...
#'   \code{\link{ctdna_filter_apply}} as a pre-filter on the genomic
...
ctdna_assemble_modalities <- function(ctdna = NULL,
  ...
  # Filtering is now a separate user step — call ctdna_filter_apply()
  ...
  # function in v0.26.0 — call ctdna_filter_apply() explicitly afterward.
  if (isTRUE(cleanup)) {
    for (m in names(inputs))
  ...
  if (!is.null(filter_scheme)) {
    .ctdna_warn_once("assemble_modalities_filter_scheme", paste0(
      ...
      "call ctdna_filter_apply() explicitly on the modalities you need."))
    for (m in c("genomic_74","genomic_500","mutations")) {
      if (m %in% names(inputs))
        inputs[[m]] <- ctdna_filter_apply(inputs[[m]],
...
#' }
#'
#' @param prep Output of \code{\link{ctdna_prepare}()}, or any
#'   list-like with the conventional modality names
#'   (\code{ctdna}, \code{tmb}, \code{ihc}, \code{expression},
#'   \code{genomic_74}, \code{genomic_500}, \code{infinity_report},
#'   \code{clinical}).
...
ctdna_pipeline <- function(prep,
  ...
  for (key in which) {
    ...
    miss   <- needs[!have]
    if (length(miss)) {
      reason <- sprintf("missing modality: %s",
                         paste(miss, collapse = ", "))
    ...
    # Build args: spec defaults + scheme + user overrides
    args <- tryCatch(spec$args(prep, scheme),
    ...
    # Drop kwargs the target function doesn't accept (allows the
    # pipeline to evolve without breaking on stray args).
    ...
    # Extract: most plots return a ggplot directly; mr_recist returns
    # a list with $plot and $table_plot; alteration_grid returns
    # list($plot,...); oncoprint returns a ctdna_oncoprint object.
    extracted <- spec$extract %||% identity
    val <- extracted(res)
    if (is.list(val) && !inherits(val, "gg") &&
        !inherits(val, "ctdna_oncoprint")) {
      # multi-piece extract: install each piece under <key>__<sub>
      for (sub in names(val)) {
        full_key <- if (sub == "plot") key else paste(key, sub, sep = "__")
        out$plots[[full_key]] <- val[[sub]]
      }
    } else {
      out$plots[[key]] <- val
    }
...
print.ctdna_pipeline_result <- function(x, ...) {
  ...
  cat("\n  Access plots via $plots$<key>; available keys:\n   ",
...
.pipeline_registry <- function() {
  list(
    baseline = list(
      ...
      needs  = "ctdna",
      args   = function(prep, scheme) list(
        df     = prep$ctdna,
        group  = "RECIST",
        scheme = scheme,
    ...
    baseline_dose_recist = list(
      fn     = "ctdna_plot_baseline_dose_recist",
      needs  = "ctdna",
      args   = function(prep, scheme) list(
        df     = prep$ctdna,
        scheme = scheme,
    ...
    mr_recist = list(
      fn      = "ctdna_plot_mr_recist",
      needs   = "ctdna",
      args    = function(prep, scheme) list(
        df    = prep$ctdna,
        title = "D4 - MR / RECIST concordance"),
      # mr_recist returns list(plot, table_plot, table, ...); install
      # both ggplots under mr_recist and mr_recist__table.
      extract = function(out) list(
        plot       = out$plot,
        table      = out$table_plot)),
    longitudinal = list(
    ...
    longitudinal_recist = list(
      fn     = "ctdna_plot_longitudinal_recist",
      needs  = "ctdna",
      args   = function(prep, scheme) list(
        df     = prep$ctdna,
        scheme = scheme,
        title  = "D6 - Longitudinal by dose x RECIST")),
    reduction_vs_tumor = list(
      fn     = "ctdna_plot_reduction_vs_tumor",
      needs  = "ctdna",
      args   = function(prep, scheme) list(
        df     = prep$ctdna,
        scheme = scheme,
    ...
    mut_methyl = list(
      fn     = "ctdna_plot_mut_methyl",
      needs  = "ctdna",
      args   = function(prep, scheme) list(
        df    = prep$ctdna,
        title = "D8 - maxVAF vs methylTF")),
    ...
    vs_mutation = list(
      ...
      needs  = c("ctdna","genomic_74"),
      args   = function(prep, scheme) list(
        ctdna_df = prep$ctdna,
        mut_df   = prep$genomic_74,
        gene     = "TP53",
        panel    = "74genes",
        title    = "ctDNA vs TP53 mutation")),
    # Genomic landscape -------------------------------------------------
    oncoprint = list(
      fn     = "ctdna_plot_oncoprint",
      needs  = "infinity_report",
      args   = function(prep, scheme) list(
        df              = prep$infinity_report,
        patient_data    = prep$clinical,
        top_annotations = if (!is.null(prep$clinical))
          intersect(c("RECIST","Dose"), names(prep$clinical)) else NULL,
        scheme          = scheme,
        title           = "Genomic landscape")),
    alteration_grid = list(
      fn      = "ctdna_plot_alteration_grid",
      needs   = c("infinity_report","clinical"),
      args    = function(prep, scheme) list(
        df             = prep$infinity_report,
        patient_data   = prep$clinical,
        gene_sets      = list(
