...
.defaults <- list(
  ...
  indication    = "indication",            # v0.78.0: renamed from `cancertype`;
                                           # default is now the canonical
                                           # per-patient indication label. The
                                           # old key name `cancertype` still
  ...
  # ---- ADSL indication column priority ---------------------------------
  # When indication information needs to be derived from an ADaM/clinical
  # frame, these columns are tried in order. ADaM names first
  ...
  # ---- v0.80.0: state on the figure when indications are pooled ----
  # When a prep spans several indications but indication is not a display
  # dimension, plots append a one-line note to the caption. Set FALSE to
  # suppress it everywhere.
  warn_pooled_dims = TRUE,
...
#' }
#'
#' \strong{Restricted keys.} Most \code{ctdna_opts()} keys accept any
#' value (column-name strings, palette colours, arbitrary tags), but a
#' subset is validated at set-time. Attempting to set one of these
#' keys to a disallowed value errors.
#'
#' \emph{Restricted-value keys (must be one of the listed strings):}
#' \tabular{ll}{
#'   \code{pairwise_test}   \tab \code{"wilcox"} / \code{"t"} / \code{"kruskal"} / \code{"anova"} / \code{"auto"} \cr
#'   \code{cor_method}      \tab \code{"spearman"} / \code{"pearson"} / \code{"kendall"} \cr
#'   \code{stat_position}   \tab \code{"subtitle"} / \code{"caption"} / \code{"on_plot"} / \code{"none"} \cr
#'   \code{legend_position} \tab \code{"right"} / \code{"left"} / \code{"top"} / \code{"bottom"} / \code{"none"} \cr
#'   \code{expression_unit} \tab \code{"log2_tpm_plus_one"} / \code{"tpm"} \cr
#' }
#'
#' \emph{Boolean keys (must be \code{TRUE} or \code{FALSE}):}
#' \code{show_stats}, \code{show_n}.
#'
#' \emph{Numeric-range keys (must be a numeric scalar in the interval):}
#' \tabular{ll}{
#'   \code{display_floor} \tab \code{[1e-10, 1]} \cr
#'   \code{loq}           \tab \code{[1e-10, 1]} \cr
#'   \code{alpha}         \tab \code{[0, 1]} \cr
#'   \code{point_size}    \tab \code{[0.1, 20]} \cr
#'   \code{point_alpha}   \tab \code{[0, 1]} \cr
#'   \code{box_width}     \tab \code{[0.05, 1]} \cr
#'   \code{bar_width}     \tab \code{[0.05, 1]} \cr
#'   \code{discord_log10} \tab \code{[0, 10]} \cr
#' }
#'
#' All other keys accept any value. Call
#' \code{\link{ctdna_opts_choices}()} for the same table dumped from
#' the runtime registry (useful at the console when you want a quick
#' reminder without opening the help page).
#'
...
ctdna_opts <- function(..., load = NULL, domain = NULL, .reset = FALSE) {
  ...
  args <- list(...)
  # v0.78.0: `cancertype` was renamed to `indication`. Resolve the old name to
  # the new one on both the get and set paths so existing scripts keep working.
  args <- .opts_resolve_deprecated(args)
  if (length(args) == 0) {
...
.print_opts <- function(domain = NULL) {
  ...
  all_keys <- names(cfg)
  domain_specs <- list(
    clinical   = c("subject","indication","cohort","recist","dose","mr",
...
#' for numeric keys).
#' List the restricted-value keys in ctdna_opts()
#'
#' A handful of \code{\link{ctdna_opts}} keys accept only a restricted
#' set of values (e.g. \code{stat_position} is one of
#' \code{"subtitle"}, \code{"caption"}, \code{"on_plot"},
#' \code{"none"}). This helper reports which keys are so restricted
#' and what their allowed values are.
#'
#' @param key Optional key name. If omitted, prints the full table of
#'   restricted keys and returns \code{NULL} invisibly. When supplied,
#'   returns the allowed values as a character vector.
#' @return Invisibly, the allowed values for \code{key}, or
#'   \code{NULL} when \code{key} is not restricted or is omitted.
#' @seealso \code{\link{ctdna_opts}} (get / set values);
#'   \code{\link{ctdna_opts_save}}, \code{\link{ctdna_opts_load}}
#'   (session-round-trip configs).
#' @examples
#' # Print the full table.
#' ctdna_opts_choices()
#'
#' # Get the allowed values for one key.
#' print(ctdna_opts_choices("stat_position"))
#' @export
...
# Internal shortcut
.o <- function(k) {
  # v0.78.0: resolve deprecated key names (e.g. `cancertype` -> `indication`)
  # silently on the internal read path, so any call site not yet migrated
  # returns the right value rather than NULL.
  if (length(k) == 1L && is.character(k) && !is.null(.opts_deprecated_keys) &&
      k %in% names(.opts_deprecated_keys))
    k <- unname(.opts_deprecated_keys[[k]])
  .env$cfg[[k]]
}
...
#' @seealso \code{\link{ctdna_compute_mr}} (turns per-visit ratios
#'   into a per-subject molecular-response call);
#'   \code{\link{ctdna_summary}}; \code{\link{ctdna_prepare}},
#'   \code{\link{ctdna_sample_qc}} (upstream).
#' @examples
...
#' print(head(r[, c("Patient_ID", "Visit_name",
...
#'
#' Collapses a longitudinal per-sample ctDNA frame to one row per
#' subject with the canonical summary metrics (\code{baseline_tf},
#' \code{best_ratio}, \code{last_tf}) and any per-patient metadata
#' present on the input (RECIST, Dose, Cohort, MR, best change).
#' Serves as the join key between ctDNA and orthogonal modalities in
#' the cross-modality plots (\code{\link{ctdna_plot_vs_tmb}},
#' \code{\link{ctdna_plot_vs_ihc}}, etc.).
#'
#' @details
#' Reads the per-subject / per-visit / per-TF-column trio via the
#' current \code{ctdna_opts()} configuration (defaults:
#' \code{Patient_ID}, \code{Visit_name},
#' \code{Methylation_tumor_fraction_percentage}), then derives:
#' \itemize{
#'   \item \code{baseline_tf} -- TF at the baseline visit
#'     (\code{ctdna_opts("baseline")}, typically \code{"C1D1"}).
#'   \item \code{best_ratio} -- minimum on-treatment / baseline TF
#'     ratio per subject (deepest molecular response).
#'   \item \code{last_tf} -- TF at the chronologically last
#'     on-treatment visit.
#'   \item \code{best_lesion_change} -- the best (most-negative)
#'     RECIST target-lesion percent change, derived from the source
#'     column with fall-through preference:
#'     \code{best_lesion_change} if already broadcast onto the frame,
#'     else \code{ctdna_opts("lesion_pct")} (typically ADTR
#'     \code{PCHG}), else computed from the per-visit lesion-size
#'     column \code{ctdna_opts("lesion")}.
#'   \item Any per-patient metadata columns already present:
#'     \code{RECIST}, \code{Dose}, \code{cohort}, \code{MR},
#'     \code{best_change}.
#' }
#'
#' \code{Record_type == "tumor"} rows (imaging-derived rows sometimes
#' interleaved on the samples frame) are dropped before the summary
#' is computed.
#'
#' @param df A longitudinal ctDNA data frame -- typically
#'   \code{prep$samples} from \code{\link{ctdna_prepare}} after
#'   \code{\link{ctdna_sample_qc}}, but any frame carrying the
#'   canonical subject / visit / TF columns works.
#' @return A data frame with one row per subject; column order is
#'   subject-ID first, followed by \code{baseline_tf},
#'   \code{best_ratio}, \code{last_tf}, \code{best_lesion_change}
#'   (when derivable), and any preserved per-patient metadata columns.
#'
#' @seealso \code{\link{ctdna_metric_at}} (more flexible per-subject
#'   extraction at a chosen visit and metric);
#'   \code{\link{ctdna_merge_modalities}} (join the summary with
#'   another modality by subject);
#'   \code{\link{ctdna_prepare}}, \code{\link{ctdna_sample_qc}}
...
#' prep <- ctdna_sample_qc(prep, verbose = FALSE)
#'
#' # One row per patient with baseline_tf / best_ratio / last_tf +
#' # preserved metadata columns (RECIST, Dose, etc.).
#' summ <- ctdna_summary(prep$samples)
#' print(head(summ))
#' @export
...
#' @seealso \code{\link{ctdna_ratio}} (per-visit ratio inputs);
#'   \code{\link{ctdna_summary}} (per-subject summary that also
#'   computes \code{best_ratio}); \code{\link{ctdna_plot_reduction}}
#'   (visualise the MR calls).
#' @examples
...
#' prep <- ctdna_sample_qc(prep, verbose = FALSE)
#'
#' # Best-ratio MR (default): the deepest drop across all on-treatment
#' # visits per subject.
#' print(head(ctdna_compute_mr(prep$samples)))
#'
#' # MR at a specific visit.
#' print(head(ctdna_compute_mr(prep$samples, at = "C5D1")))
#'
#' # Stricter threshold (70% drop required).
#' print(head(ctdna_compute_mr(prep$samples, threshold = 0.3)))
#' @export
...
#' @param tf_col Name of the tumour-fraction column to use. \code{NULL}
#'   (default) resolves to \code{ctdna_opts("tf")}, the canonical
#'   methylation-TF column. Pass an explicit column name to compute
#'   the metric on a different TF (e.g. \code{"maxVAF"} for VAF-based
#'   reduction).
#' @return Data frame: one row per subject, with columns
...
#' prep <- ctdna_sample_qc(prep, verbose = FALSE)
#'
#' # Default -- best-ratio per subject (deepest TF drop)
#' print(head(ctdna_metric_at(prep$samples)))
#'
#' # TF at C5D1
#' print(head(ctdna_metric_at(prep$samples, metric = "tf", at = "C5D1")))
#'
#' # Percent change at the last on-treatment visit
#' print(head(ctdna_metric_at(prep$samples,
#'   metric = "pct_change", at = "last")))
#' @export
...
#' @seealso \code{\link{ctdna_sample_qc}} (the higher-level QC entry
#'   point that applies these thresholds across all modality frames
#'   inside a \code{ctdna_prep}); \code{\link{ctdna_prepare}}.
#' @examples
#' sim  <- ctdna_make_mock_study(n_patients = 20, seed = 1)
#' prep <- ctdna_prepare(
#'   infinity_report = sim$infinity_report,
#'   adam            = list(adsl = sim$clinical),
#'   verbose         = FALSE)
#' # Cleanup on the samples frame directly (kind is auto-detected).
#' cleaned <- ctdna_cleanup(prep$samples, quiet = TRUE)
#' cat(sprintf("Before: %d rows / After: %d rows\n",
#'             nrow(prep$samples), nrow(cleaned)))
#'
#' # Pass an explicit kind if you know the shape.
#' cleaned2 <- ctdna_cleanup(prep$samples, kind = "ctdna", quiet = TRUE)
#' print(head(cleaned2[, c("Patient_ID", "Sample_ID", "Visit_name",
#'                         "ctDNA_detection_status")]))
#' @export
...
.ctdna_filter_block_catalog <- function() {
  out <- list()
  # Indication x gene_set: derive from the existing builtin templates
...
#' an RDS file. \code{ctdna_opts_load(file)} restores it. Together
#' they let a study configuration be captured once, checked into a
#' repository, and reloaded in every downstream session without
#' having to reproduce a chain of \code{ctdna_opts(...)} setters by
#' hand.
#'
#' Opts are session-global -- there is only one set at a time -- so
#' neither function takes a name argument. The saved payload is
#' tagged \code{"ctdnaTM_opts_payload"} so \code{ctdna_opts_load}
#' can refuse to consume a filter-scheme payload written by a
#' different save function.
#'
#' @param file RDS file path.
#' @return Invisibly, the file path (for save) or the loaded opts
#'   list (for load).
#' @seealso \code{\link{ctdna_opts}} (get / set individual options);
#'   \code{\link{ctdna_opts_choices}} (list keys with a restricted
#'   set of allowed values).
#' @examples
#' # Configure a couple of display options, save, reset, reload.
#' ctdna_opts(stat_position = "caption", legend_position = "bottom")
#' f <- tempfile(fileext = ".rds")
#' ctdna_opts_save(f)
#'
#' ctdna_opts(.reset = TRUE)
#' print(ctdna_opts("stat_position"))    # default value
#'
#' ctdna_opts_load(f)
#' print(ctdna_opts("stat_position"))    # "caption" -- restored
#'
#' # Clean up.
#' ctdna_opts(.reset = TRUE)
#' unlink(f)
...
#' @seealso \code{\link{ctdna_prepare}} (the current entry point that
#'   handles vendor input directly and produces a \code{ctdna_prep}
#'   object; recommended for new code); \code{\link{ctdna_merge_modalities}};
#'   \code{\link{ctdna_opts}} (column-name customisation).
#' @examples
#' # ctdna_assemble_modalities is the legacy multi-modal assembler
#' # (pre-dating ctdna_prepare). It takes one canonical-column frame
#' # per modality and returns a validated list. For new code, use
#' # ctdna_prepare() which handles the vendor input directly.
...
#'   verbose         = FALSE)
#'
#' # prep$samples already carries the canonical column names, so it
#' # can be handed to ctdna_assemble_modalities as the ctdna frame.
#' # strict = FALSE lets missing/extra columns pass with a warning.
#' ms <- ctdna_assemble_modalities(ctdna = prep$samples, strict = FALSE)
#' print(names(ms))
#' @export
...
.gh_build_variant <- function(variant_type, sample_tf_pct, force_gene = NULL) {
  # force_gene lets the caller plant a specific gene so the same gene can be
  # seeded in both the plasma and tissue mocks for the same patient, producing
  # genuine cross-modality concordance. NULL keeps the original random draw.
  g <- if (!is.null(force_gene)) force_gene else sample(.gh_genes_74, 1)
  row <- list(
...
ctdna_make_mock_study <- function(n_patients   = 30,
  ...
  # ---- Cross-modality concordance seeding -----------------------------------
  # Plasma and tissue alterations were drawn independently, so agreement
  # between the two modalities was close to chance and the concordance
  # oncoprint had almost no matching pairs. Two designated genes are now
  # planted in BOTH modalities for the same subset of patients, giving a
  # visible concordant signal to look at while every other gene keeps its
  # independent draw.
  .concordant_genes <- intersect(c("TP53", "PTEN"), .gh_genes_74)
  if (!length(.concordant_genes)) .concordant_genes <- utils::head(.gh_genes_74, 2)
  # ~55% of patients carry the shared alteration in each designated gene.
  pts$.conc_g1 <- stats::runif(n_patients) < 0.55
  pts$.conc_g2 <- stats::runif(n_patients) < 0.55

  # ============================================================
  # Sample master table (one row per Patient x Visit)
  # ============================================================
  ...
  for (si in seq_len(nrow(samples))) {
    ...
    # LGR was 0.05 and so rarely survived filtering that it never appeared in
    # the oncoprints. Raised so the class is visible; the extra weight comes
    # off SNV, which was over-represented.
    alt_types <- sample(c("SNV","Indel","CNV","Fusion","LGR"),
                        n_alt, replace = TRUE,
                        prob = c(0.50, 0.11, 0.16, 0.11, 0.12))
    # Plant the designated concordant genes for this patient (baseline visit
    # only, which is what the landscape functions read) so the same genes are
    # altered in plasma AND tissue. Appended to the random draw rather than
    # replacing it.
    # This loop iterates SAMPLES (sr), not patients, so the per-patient flags
    # are looked up by Patient_ID. Planted at the baseline visit only, which is
    # what the landscape / concordance functions read.
    forced_g <- character(0)
    .pi <- match(sr$Patient_ID, pts$Patient_ID)
    if (!is.na(.pi) && identical(as.character(sr$Visit_name), "C1D1")) {
      if (isTRUE(pts$.conc_g1[.pi]) && length(.concordant_genes) >= 1L)
        forced_g <- c(forced_g, .concordant_genes[1])
      if (isTRUE(pts$.conc_g2[.pi]) && length(.concordant_genes) >= 2L)
        forced_g <- c(forced_g, .concordant_genes[2])
    }
    alt_types <- c(alt_types, rep("SNV", length(forced_g)))
    forced_vec <- c(rep(NA_character_, length(alt_types) - length(forced_g)),
                     forced_g)

    for (.ai in seq_along(alt_types)) {
      alt_t <- alt_types[.ai]
      .fg <- forced_vec[.ai]
      variant <- .gh_build_variant(alt_t, sr$TF_pct,
                                   force_gene = if (is.na(.fg)) NULL else .fg)
      inf_rows[[ridx]] <- as.data.frame(c(
        common_id,
  ...
  # ============================================================
  # IHC (per patient x marker, single baseline tissue sample)
  # ============================================================
  ihc <- NULL
  if (.keep_ihc) {
  ihc_rows <- list()
  ...
  for (i in seq_len(nrow(pts))) {
    ...
    # Tissue alteration mix. "SV" produced rows with no CNV_type and no
    # Molecular_consequence, so every non-SNV/Indel tissue row classified as
    # "Other" and the tissue side of the concordance oncoprint was dominated by
    # it. Replaced with LGR (a class the package actually models) and given
    # real weight so LGR is visible in both oncoprints.
    var_types <- sample(c("SNV","Indel","CNV","Fusion","LGR"),
                        n_var, replace = TRUE,
                        prob = c(0.46, 0.15, 0.18, 0.09, 0.12))
    # Same designated genes as the plasma loop for the same patients, so those
    # genes read as concordant across the two modalities.
    forced_g <- character(0)
    if (isTRUE(pat$.conc_g1) && length(.concordant_genes) >= 1L)
      forced_g <- c(forced_g, .concordant_genes[1])
    if (isTRUE(pat$.conc_g2) && length(.concordant_genes) >= 2L)
      forced_g <- c(forced_g, .concordant_genes[2])
    var_types <- c(var_types, rep("SNV", length(forced_g)))
    forced_vec <- c(rep(NA_character_, length(var_types) - length(forced_g)),
                     forced_g)

    for (.vi in seq_along(var_types)) {
      vt <- var_types[.vi]
      .fg <- forced_vec[.vi]
      g <- if (!is.na(.fg)) .fg
           else sample(c(.gh_genes_74, .gh_genes_500_only), 1)
      ...
      wes_rows[[widx]] <- data.frame(
        ...
        # CNV_type is what the landscape classifier reads to resolve a CNV into
        # Amp / Focal_Amp / Homozygous_Del / LOH. Without it CNV rows fell
        # through to "Other".
        CNV_type = if (vt == "CNV") sample(
          c("focal_amplification","amplification",
            "homozygous_deletion","loh_deletion"),
          1, prob = c(0.28, 0.22, 0.28, 0.22)) else NA_character_,
        # LGR direction pair -- deletion / tandem duplication / inversion.
        Direction_a = if (vt == "LGR") sample(c(-1L, 1L), 1) else NA_integer_,
        Direction_b = if (vt == "LGR") sample(c(-1L, 1L), 1) else NA_integer_,
        stringsAsFactors = FALSE
      )
      widx <- widx + 1
    }
  ...
  # Patient x gene effects driven by RECIST for immune genes
...
#'
#' Builds every registered deliverable in one call from a prepared
#' \code{ctdna_prep} object. Each step is opt-in via the modalities
#' present on the prep: steps whose required frame is missing are
#' skipped with a clear reason; steps that error are captured (never
#' halt the pipeline) with the full error message for later review.
#'
#' @details
#' \strong{The registry.} A single internal step registry maps every
#' short name (\code{"baseline"}, \code{"reduction"},
#' \code{"longitudinal"}, \code{"oncoprint"}, ...) to (a) the plot
#' function it calls, (b) the prep slot(s) it needs, and (c) the
#' default arguments it uses. \code{\link{ctdna_pipeline_steps}}
#' returns this as a data frame so callers can see the current
#' catalog without running anything. \code{which =} takes any subset
#' of those keys.
#'
#' \strong{No silent failures.} Every step either lands in
#' \code{$plots$<key>} (success) or in one of two named slots:
#' \code{$failures$<key>} if the plot function errored (value = the
#' exact error message), or \code{$skipped$<key>} if a required prep
#' slot was missing (value = a string naming the missing slot).
#'
#' \strong{Per-plot overrides.} Every plot's registry defaults can be
#' overridden with \code{overrides = list(<key> = list(...))}. For
#' example \code{overrides = list(baseline = list(method = "maxvaf"))}
#' passes \code{method = "maxvaf"} into \code{ctdna_plot_baseline()}
#' while leaving every other step at its default.
#'
#' @param prep A \code{ctdna_prep} object from
#'   \code{\link{ctdna_prepare}}, typically after
#'   \code{\link{ctdna_sample_qc}} and (optionally)
#'   \code{\link{ctdna_variant_filter}}. Registered steps consume the
#'   standard slots (\code{$samples}, \code{$variants},
#'   \code{$clinical}, \code{$assessments}) plus any optional
#'   cross-modality frames the prep carries (\code{$ihc},
#'   \code{$dnaseq}, \code{$rnaseq}).
#' @param which Character vector of pipeline keys to run. \code{NULL}
#'   (default) runs every deliverable whose required modalities are
#'   present. Call \code{ctdna_pipeline_steps()} for the full catalog
#'   of keys.
#' @param scheme Default RECIST stratification scheme used by every
#'   step that takes a \code{scheme =} argument. One of
...
#'   \code{visit =} argument (oncoprint, alteration_grid). Default
#'   \code{"baseline"} (case-insensitive; also accepts
#'   \code{"base"}, \code{"screening"}, or a canonical visit label
#'   like \code{"C1D1"}).
...
#'   one (contingency comparisons). \code{"fisher"} (default) or
#'   \code{"chisq"}.
...
#' @param overrides Named list of per-step keyword-argument lists.
#'   Keys are pipeline keys; values are arg lists that override the
#'   registry defaults for that step. Example:
#'   \code{list(baseline = list(method = "maxvaf"),
#'              reduction = list(metric = "tf"))}.
#' @param compose If \code{TRUE}, also build a \code{patchwork}
#'   composite of every plot that succeeded and return it as
#'   \code{$composite}. Default \code{FALSE}.
#' @param verbose Logical. When \code{TRUE} (default), prints step-
#'   by-step progress including the argument set each step will use
#'   (helpful for debugging overrides).
#' @param dry_run If \code{TRUE}, does not build any plots -- returns
#'   a list reporting which steps would run and which would be
#'   skipped (and why). Useful for confirming what the pipeline
#'   \emph{would} do against the current prep.
#' @param indication Optional indication (cohort) restriction applied before
#'   any step runs. A single string or a character vector of Indication
#'   values, e.g. \code{indication = "HNSCC"} or
#'   \code{indication = c("HNSCC","ES_SCLC")}. \code{NULL} (the default) uses
#'   every indication present in the data. Values are matched against the
#'   canonical \code{Indication} column, resolved via
#'   \code{ctdna_opts("indication")} with fallbacks to \code{Indication},
#'   \code{indication}, \code{Cancertype} and \code{Cancer_Type}. This is the
#'   modern spelling of the historical \code{cancer_type=} argument.
#' @param indications Deprecated alias for \code{indication}; still honoured.
#'   If both are supplied and they differ, \code{indication} wins.
#'
#' @return An object of class \code{ctdna_pipeline_result} -- a
#'   named list with:
#'   \itemize{
#'     \item \code{$plots} -- named list of plot objects, one per
#'       successful step. Type depends on the step: ggplot for
#'       trajectory / cross-modality plots, ComplexHeatmap or ggplot
#'       for the landscape (see \code{engine}), a small list
#'       (\code{plot / summary / stats}) for
#'       \code{ctdna_alteration_grid}.
#'     \item \code{$failures} -- named list of error-message strings
#'       for steps that errored. Empty when everything succeeded.
#'     \item \code{$skipped} -- named list of "missing slot" reason
#'       strings for steps whose required modality wasn't in the
#'       prep. Empty when the prep covers every registered step.
#'     \item \code{$timing} -- named numeric vector of seconds per
#'       step.
#'     \item \code{$composite} -- a patchwork composite of every
#'       successful ggplot. Only present when \code{compose = TRUE}.
#'   }
#'   For convenience each successful step also appears at the top
#'   level (\code{result$baseline}, \code{result$oncoprint}, ...) in
#'   addition to \code{result$plots$<key>}.
#'
#' @seealso \code{\link{ctdna_pipeline_steps}} (the registered
#'   step catalog); \code{\link{ctdna_prepare}} and
#'   \code{\link{ctdna_sample_qc}} (upstream requirements);
#'   \code{\link{ctdna_variant_filter}} (variant filter to apply
#'   before running the pipeline);
#'   \code{\link{ctdna_run_pipeline}} (legacy wrapper taking
#'   individual modality frames rather than a prep).
#'
#' @examples
#' sim  <- ctdna_make_mock_study(n_patients = 20, seed = 1)
#' prep <- ctdna_prepare(
...
#' prep <- ctdna_sample_qc(prep, verbose = FALSE)
#'
#' # Run every step whose modalities are present in this minimal prep.
#' out <- ctdna_pipeline(prep, verbose = FALSE)
#' print(names(out$plots))    # every successful step
#' print(out$skipped)         # which steps were skipped (and why)
#' print(out$timing)          # seconds per step
#'
#' # Run only a subset, with per-step overrides:
#' out_sub <- ctdna_pipeline(prep,
#'   which     = c("baseline", "reduction"),
#'   overrides = list(baseline  = list(method = "maxvaf"),
#'                    reduction = list(metric = "tf")),
#'   verbose   = FALSE)
#' print(names(out_sub$plots))
#'
#' # Dry-run: see what would run without building anything.
...
ctdna_pipeline <- function(prep,
                            ...
                            indication    = NULL,
                            indications   = NULL) {
  # v0.79.0: `facet` is an alias for `wrap`.
  wrap <- .ctdna_resolve_facet(facet, wrap, "ctdna_pipeline")
  prep <- .ctdna_filter_prep_by_indication(prep, .ctdna_resolve_indication(indication, indications))
  if (!is.list(prep))
  ...
  list(
    ...
    vs_mutation = list(
      ...
      args   = function(prep, opts) list(
        ...
        gene     = "TP53",
        # v0.76.2: stratify by alteration_type (Variant_type: SNV / CNV /
        # Fusion / LGR / Indel) rather than the deprecated mutation_status
        # column. prepare() no longer synthesises a per-patient mut / wt
        # flag; alteration_type is present verbatim on prep$variants.
        by       = "alteration_type",
        title    = "ctDNA vs TP53 alteration type")),
...
# ============================================================================
# .opts_deprecated_keys / .opts_resolve_deprecated()
# ----------------------------------------------------------------------------
# v0.78.0: `cancertype` was renamed to `indication` (and its default changed
# from "Cancertype" to "indication"). Old scripts that call
# ctdna_opts(cancertype = ...) or ctdna_opts("cancertype") keep working: the
# old name is transparently resolved to the new key, with a message so the
# user can migrate.
#
# `.opts_deprecated_keys` is old_name -> new_name. Add future renames here.
# ============================================================================
.opts_deprecated_keys <- c(cancertype = "indication")

.opts_resolve_deprecated <- function(args) {
  if (!length(args)) return(args)
  old <- names(.opts_deprecated_keys)

  # --- set path: named arguments, e.g. ctdna_opts(cancertype = "x") ---
  nm <- names(args)
  if (!is.null(nm)) {
    hit <- nm %in% old
    if (any(hit)) {
      for (i in which(hit)) {
        from <- nm[i]; to <- unname(.opts_deprecated_keys[[from]])
        message(sprintf(
          "ctdna_opts: `%s` is deprecated and was renamed to `%s` in v0.78.0. Using `%s`.",
          from, to, to))
        nm[i] <- to
      }
      names(args) <- nm
  ...
  # --- get path: unnamed character keys, e.g. ctdna_opts("cancertype") ---
  if (is.null(names(args)) || all(names(args) == "")) {
    vals <- unlist(args, use.names = FALSE)
    if (is.character(vals) && any(vals %in% old)) {
      for (i in seq_along(vals)) {
        if (vals[i] %in% old) {
          from <- vals[i]; to <- unname(.opts_deprecated_keys[[from]])
          message(sprintf(
            "ctdna_opts: `%s` is deprecated and was renamed to `%s` in v0.78.0. Reading `%s`.",
            from, to, to))
          vals[i] <- to
        }
      }
