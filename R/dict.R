...
#' \code{dnaseq_dict("sub.entry" = aliases)}. Appends one or more raw
#' vendor strings to an existing canonical entry so downstream
#' normalisation (inside \code{\link{ctdna_prepare}} /
#' \code{\link{ctdna_prep_add}}) recognises those strings.
#' Duplicates are removed case-insensitively, so it is safe to call
#' multiple times with overlapping alias sets.
#'
#' @param sub One of \code{"variant_type"}, \code{"molecular_consequence"},
#'   \code{"cnv_type"}.
#' @param canonical The canonical entry name (must already exist under
#'   \code{sub}). See \code{\link{dnaseq_dict}} for the full canonical
#'   universe.
#' @param aliases Character vector of raw vendor strings to add.
#'
#' @return The updated dictionary, invisibly.
#' @seealso \code{\link{dnaseq_dict}} (get / set / list the whole dict);
#'   \code{\link{dnaseq_dict_reset}} (restore shipped defaults);
#'   \code{\link{dnaseq_dict_lookup}} (resolve raw values to canonical
#'   names).
#' @examples
#' # Restore defaults so this example is idempotent.
#' dnaseq_dict_reset()
#'
#' # Register a vendor-specific string for the "missense" canonical entry.
#' dnaseq_dict_add("molecular_consequence", "missense", "MyVendor_Missense")
#'
#' # Verify: the alias now resolves to "missense".
#' print(dnaseq_dict_lookup("molecular_consequence", "MyVendor_Missense"))
#'
#' # Clean up.
#' dnaseq_dict_reset()
#' @export
...
#'
#' Discards any vendor-specific aliases added via
#' \code{\link{dnaseq_dict}} or \code{\link{dnaseq_dict_add}} in the
#' current session and reinstates the factory-shipped canonical map.
#' Useful for making examples reproducible, and when re-running a
#' pipeline after an alias load-in that turned out to be too
#' aggressive.
#'
#' @return The (now-default) dictionary, invisibly.
#' @seealso \code{\link{dnaseq_dict}}, \code{\link{dnaseq_dict_add}},
#'   \code{\link{dnaseq_dict_lookup}}.
#' @examples
#' # Add a custom alias.
#' dnaseq_dict_add("variant_type", "SNV", "MyVendorSNP")
#' print(dnaseq_dict_lookup("variant_type", "MyVendorSNP"))  # "SNV"
#'
#' # Reset removes it.
#' dnaseq_dict_reset()
#' print(dnaseq_dict_lookup("variant_type", "MyVendorSNP"))  # NA
#' @export
...
#'
#' @seealso \code{\link{dnaseq_dict}}, \code{\link{dnaseq_dict_add}},
#'   \code{\link{dnaseq_dict_reset}}.
#'
#' @examples
#' # variant_type: real vendor strings resolve to canonical names.
#' # Unmapped strings return NA (per @return contract).
#' print(dnaseq_dict_lookup("variant_type",
#'   c("point_mutation", "INDEL", "CNA", "gene_fusion",
#'     "large_rearrangement", "banana")))
#' # -> c("SNV", "Indel", "CNV", "Fusion", "LGR", NA)
#'
#' # molecular_consequence: combined "+" strings split; most-damaging wins.
#' print(dnaseq_dict_lookup("molecular_consequence",
#'   c("missense_variant",
#'     "frameshift_variant+splice_donor_variant+intron_variant",
#'     "STOP GAINED",
#'     "banana")))
#' # -> c("missense", "frameshift", "nonsense", NA)
#'
#' # cnv_type: canonicalises amp / homozyg-del / LOH spellings.
#' print(dnaseq_dict_lookup("cnv_type",
#'   c("AMP", "GAIN", "hom_del", "LOH", "banana")))
#' # -> c("focal_amplification", "amplification",
#' #      "homozygous_deletion", "loh_deletion", NA)
#'
#' # NA and empty strings pass through as NA.
#' print(dnaseq_dict_lookup("variant_type", c(NA, "", "SNV")))
#' @export
