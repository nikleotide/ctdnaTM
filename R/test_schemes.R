...
# what ctdna_filter_apply(..., filter_scheme = scheme_name) would do for
...
ctdna_test_scheme <- function(scheme_name,
  ...
  if (isTRUE(verify)) {
    ...
    out <- tryCatch(
      ctdna_filter_apply(df, filter_scheme = scheme_name, verbose = FALSE),
      error = function(e) {
