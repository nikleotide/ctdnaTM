...
# `protect` = character vector of columns the satisfier must NOT overwrite
# (because higher-level gates already populated them). Cross-column ops
# (==col / !=col) invert their assignment direction when the primary column
# is protected so the partner column is adjusted instead.
.ctdna_test_satisfy_atom <- function(row, atom, protect = character(0)) {
  for (col_name in names(atom$conditions)) {
    ...
    if (is.list(constraint) && all(c("op","value") %in% names(constraint))) {
      ...
      if (op == "==col") {
        ...
        # If col_name is protected, invert: set the partner column instead so
        # the protected column keeps the value the gate assigned it.
        if (col_name %in% protect) {
          row[[val]] <- row[[col_name]]
        } else {
        ...
        # If col_name is protected, invert: pick a miss for the partner.
        if (col_name %in% protect) {
          row[[val]] <- .ctdna_test_pick_miss(val, row[[col_name]])
        } else {
          other_val <- row[[val]]
          row[[col_name]] <- .ctdna_test_pick_miss(col_name, other_val)
        }
      } else if (col_name %in% protect) {
        # Non-cross-column op on a protected column: leave it alone. The
        # gate is responsible for populating it compatibly.
        next
      ...
      # Plain value(s) -- pick the first (skip if protected)
...
.ctdna_test_satisfy <- function(row, node, protect = character(0)) {
  ...
  if (inherits(node, "ctdna_rule_atom"))
    return(.ctdna_test_satisfy_atom(row, node, protect))
  if (inherits(node, "ctdna_rule_combinator")) {
...
ctdna_test_scheme <- function(scheme_name,
  ...
  if (isTRUE(verify)) {
    ...
    # v0.76.2: ctdna_variant_filter() has been prep-only since v0.71, so
    # calling it on a raw data frame errors. The synthetic test frame IS
    # a data frame, so drop to the internal data-frame API that still
    # honours the same scheme evaluator.
    out <- tryCatch(
      .filter_apply_df(df, filter_scheme = scheme_name, verbose = FALSE),
      error = function(e) {
