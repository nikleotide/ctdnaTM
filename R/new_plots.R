...
# v0.76.2: read the per-patient indication column with the SAME fall-through
# chain already used by .ctdna_filter_prep_by_indication() so a user's data
# with `Cancertype` (the natural ADaM label) works without hand-adding an
# `indication` column. Search order matches the helper in zzz.R:
#     Indication -> indication -> Cancertype -> Cancer_Type
.read_indication_col <- function(prep, subj_col = "Patient_ID") {
  for (nm in c("Indication", "indication", "Cancertype", "Cancer_Type")) {
    v <- .read_per_patient(prep, nm, subj_col = subj_col)
    if (!is.null(v) && any(!is.na(v) & nzchar(v))) return(v)
  }
  NULL
}
...
.build_landscape_base <- function(prep, filter_scheme, visit, visit_col,
  ...
  # ---- Indication (required) ----
  # v0.76.2: try the full documented fall-through chain
  # (Indication -> indication -> Cancertype -> Cancer_Type) rather than
  # the single "indication" literal. Matches the behaviour of the
  # `indications` filter argument.
  ind <- .read_indication_col(prep, subj_col = patient_col)
  if (is.null(ind))
    stop(fn_label, ": no indication column found in prep$adrs, any ADaM ",
         "frame, prep$variants, or prep$clinical (looked for: Indication, ",
         "indication, Cancertype, Cancer_Type). Add one covering every ",
         "patient and try again.", call. = FALSE)
...
#'   axis. Default \code{TRUE}. Set \code{FALSE} for large cohorts
#'   where IDs overlap.
#' @param show_freq_bar Logical. Draw the per-gene alteration-
#'   frequency bar on the right of the matrix. Default \code{TRUE}.
#' @param engine `"auto"` (CH if installed, else ggplot), `"complexheatmap"`,
#'   or `"ggplot"`.
#' @param title Main title. \code{NULL} = no title.
#' @param subtitle Subtitle text under the title. \code{NULL} = none.
#' @param caption Caption below the plot. \code{NULL} = none.
#' @param patient_col Name of the patient-ID column on
#'   \code{prep$variants} (default \code{"Patient_ID"}).
#' @param gene_col Name of the gene-symbol column on
#'   \code{prep$variants} (default \code{"Gene"}).
#' @param variant_col Optional override for the variant/alteration
#'   column. \code{NULL} (default) auto-detects from
#'   \code{prep$variants} in the standard order (\code{Variant_type},
#'   \code{alteration_type}, ...).
...
#'   (default) = no filter.
#' @return A `ctdna_oncoprint` object. Print it with \code{print()} to
#'   render -- the object carries either a \code{ComplexHeatmap}
#'   heatmap object (when the engine is \code{"complexheatmap"}) or a
#'   \code{ggplot} at \code{$plot} (when the engine is
#'   \code{"ggplot"}), together with the resolved gene sets,
#'   annotation values, and legend state used for rendering.
#'
#' @seealso \code{\link{ctdna_alteration_grid}} (same patient base,
#'   stacked-bar view of gene-set membership by response);
#'   \code{\link{ctdna_alteration_types}} (canonical alteration-class
#'   names for the \code{alterations} argument);
#'   \code{\link{ctdna_gene_set}} (built-in gene sets like \code{"HRR14"}).
#'
#' @examples
...
#'
#' # Baseline oncoprint of a core driver set (all in the 74-gene panel).
#' op1 <- ctdna_oncoprint(prep,
#'   gene_sets = list(Drivers =
#'     c("EGFR","KRAS","BRAF","PTEN","TP53","MET","ERBB2","CDKN2A")))
#' print(op1)
#'
#' # Multiple gene sets rendered as row splits, panels split by dose.
#' op2 <- ctdna_oncoprint(prep,
#'   gene_sets = list(
#'     TSG = c("TP53","PTEN","CDKN2A"),
#'     RTK = c("EGFR","KRAS","BRAF","MET","ERBB2")),
#'   wrap  = "Dose")
#' print(op2)
#'
#' # Restrict alteration classes on display (drop noise categories):
#' op3 <- ctdna_oncoprint(prep,
#'   gene_sets   = list(Drivers =
#'     c("EGFR","KRAS","BRAF","PTEN","TP53","MET","ERBB2","CDKN2A")),
#'   alterations = setdiff(ctdna_alteration_types(),
#'                         c("Other", "Promoter")))
#' print(op3)
#' @export
ctdna_oncoprint <- function(prep,
  ...
  facet <- .ctdna_resolve_facet(facet, wrap, "ctdna_oncoprint")
  wrap  <- facet
...
#' @param title Main title. Default
#'   \code{"Combinatorial alteration status"}.
...
#'     \item \code{$summary} -- one row per (gene set, response category,
#'       optional wrap-facet) reporting patient counts and percentages.
#'     \item \code{$stats} -- per-panel test results (Fisher's exact by
#'       default) with p-values comparing alteration prevalence across
#'       response categories.
#'   }
#'
#' @seealso \code{\link{ctdna_oncoprint}} (same patient base, per-gene
#'   matrix view); \code{\link{alt_any}}, \code{\link{alt_all}},
#'   \code{\link{alt_any_n}} (gene-set combine wrappers).
#'
#' @examples
#' sim  <- ctdna_make_mock_study(n_patients = 40, seed = 1)
#' prep <- ctdna_prepare(
...
#' prep <- ctdna_sample_qc(prep, verbose = FALSE)
#'
#' # One row per gene set; bars split by RECIST.
#' res <- ctdna_alteration_grid(prep,
#'   gene_sets = list(
#'     TSG = c("TP53","PTEN","CDKN2A"),
#'     RTK = c("EGFR","KRAS","BRAF","MET")),
#'   scheme = "three")
#' print(res$plot)
#' print(res$summary)
#' print(res$stats)
...
ctdna_alteration_grid <- function(prep,
                                  ...
                                  indications = NULL) {
  # v0.79.0: `wrap` is an alias for `facet` (and vice versa).
  facet <- .ctdna_resolve_facet(facet, wrap, "ctdna_alteration_grid")
  wrap  <- facet
