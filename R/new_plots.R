...
.resolve_visit_baseline <- function(visit) {
  ...
  baseline_hit <- tolower(trimws(v)) %in% c("baseline", "base", "screening")
  v[baseline_hit] <- "C1D1"
  v
}
...
.build_landscape_base <- function(prep, filter_scheme, visit, visit_col,
  ...
  if (is.null(dose))
    stop(fn_label, ": no column literally named 'dose' ",
         "(case-insensitive) found in any frame of `prep`. Please add a ",
         "`dose` column covering every patient and try again.",
         call. = FALSE)

  # ---- RECIST (optional here -- grid will further enforce) ----
  rec <- .read_per_patient(prep, "RECIST", subj_col = patient_col)

  # ---- drop patients missing Indication OR Dose ----
  ind_b  <- ind[base_pids]
  dose_b <- dose[base_pids]
  miss   <- is.na(ind_b) | !nzchar(ind_b) |
            is.na(dose_b) | !nzchar(dose_b)
  n_drop <- sum(miss)
  if (n_drop > 0L)
    message(sprintf(
      "%s: removed %d patient(s) missing Indication or Dose.",
      fn_label, n_drop))
  base_pids <- base_pids[!miss]
  if (!length(base_pids))
    stop(fn_label, ": every patient in the variant base is missing ",
         "Indication or Dose; nothing to plot.", call. = FALSE)
  v <- v[as.character(v[[patient_col]]) %in% base_pids, , drop = FALSE]

  patient_data <- data.frame(
    Patient_ID = base_pids,
    Indication = unname(ind[base_pids]),
    Dose       = unname(dose[base_pids]),
