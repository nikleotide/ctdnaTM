...
#' @rdname combinators
#' @examples
#' # Equivalent to anyOf(rule(Gene = "TP53"), rule(Gene = "KRAS"))
#' anyOf(rule(Gene = "TP53"), rule(Gene = "KRAS"), rule(Gene = "EGFR"))
#'
#' # Use inside a scheme
#' create_filtering_scheme(
#'   anyOf(rule(Variant_type = "SNV"), rule(Variant_type = "Indel")),
#'   name = "snv_or_indel")
#' @export
anyOf <- function(...) {
  args <- list(...)
...
#' @param branches Optional named list of \code{ctdna_rule} objects
#'   labelling the OR'd success paths of the scheme. When set, the
#'   \code{filtering_criteria} column produced by
#'   \code{\link{ctdna_variant_filter}(explain = TRUE)} reports which
#'   named branch a passing row matched, e.g.
#'   \code{"PASS::TSG_Tier1(somatic_nonsyn)"}. When \code{NULL}
#'   (default), branch labels are auto-numbered
#'   (\code{"PASS::TSG_Tier1(branch_1)"}). Used by every built-in
#'   biomarker scheme (\code{TSG_Tier1}, \code{TSG_Tier2},
#'   \code{TSG_Tier3}, \code{HRR14}) to give the explanation column
