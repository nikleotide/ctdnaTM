...
#' filtering at the top level — see \code{ctdna_filter_apply}.
#'
...
#'   \code{\link{ctdna_filter_apply}}. Default \code{"user"}.
...
create_filtering_scheme <- function(...,
  ...
  # ---- Combine template + new rules into root ------------------------------
  combined <- c(if (!is.null(tmpl_rule)) list(tmpl_rule), args)
  if (length(combined) == 0L)
    stop("`create_filtering_scheme()` requires at least one rule (positional ",
          "or via `template`).", call. = FALSE)
