...
#'
#' Boxplot of the on-treatment ctDNA metric relative to baseline,
#' split by dose (x-axis) and coloured by RECIST. Reads per-sample
#' data (\code{prep$samples}) and derives one value per patient
#' using the \code{at} + \code{metric} pair (see below). LOQ
#' exclusion is applied.
#'
#' @section Recommended statistic:
#' `"wilcox"` (default): Wilcoxon rank-sum within each dose.
#' Reduction ratios are heavily skewed even on log scale, so
#' non-parametric is the right call.
#'
#' @inheritParams ctdna_plot_baseline
#' @param at Visit selector for the on-treatment value. \code{"all"}
#'   (default) uses every on-treatment sample; \code{"best"} takes the
#'   deepest drop per patient; a literal visit label (\code{"C2D1"},
#'   \code{"Cycle 2 Day 1"}, ...) picks a specific timepoint with
#'   tolerant matching.
#' @param metric Which reduction metric to plot on the y-axis.
#'   \code{"ratio"} (default) = on-treatment / baseline TF (values
#'   below 1 mean drop, above 1 mean rise). \code{"tf"} = raw
#'   on-treatment TF. \code{"pct_change"} = \code{100 * (on-treatment
#'   - baseline) / baseline}, i.e. percent change from baseline.
#' @param mr_threshold Molecular-response cutoff drawn as a
#'   horizontal reference line when \code{metric = "ratio"} (default
#'   \code{0.5}, i.e. "\eqn{\ge}{>=}50% TF drop"). Ignored for other
#'   metrics.
...
#'
#' @seealso \code{\link{ctdna_plot_baseline}} (shared args, baseline
#'   view); \code{\link{ctdna_plot_longitudinal}} (time-course view);
#'   \code{\link{ctdna_metric_at}} (the underlying per-patient
#'   metric extractor).
#'
#' @examples
...
#'
#' # Default: best-ratio (deepest TF drop per patient), grouped by dose.
#' p1 <- ctdna_plot_reduction(prep,
#'   scheme = "three", metric = "ratio", at = "best", mr_threshold = 0.5)
#' print(p1)
#'
#' # Percent-change at a specific visit. Tolerant matching: "C2D1"
#' # and "Cycle 2 Day 1" both work.
#' p2 <- ctdna_plot_reduction(prep,
#'   scheme = "three", metric = "pct_change", at = "Cycle 2 Day 1")
#' print(p2)
#'
#' # Raw TF at the best on-treatment visit.
#' p3 <- ctdna_plot_reduction(prep,
#'   scheme = "three", metric = "tf", at = "best")
#' print(p3)
#' @export
ctdna_plot_reduction <- function(prep,
  ...
  facet <- .ctdna_resolve_facet(facet, wrap, "ctdna_plot_reduction")
  wrap  <- facet

  # ---- indications filter (v0.75.10) ----
  ...
  {
    # v0.80.0: say so on the figure when several indications are pooled
    .pn <- .ctdna_pooled_note(prep, c(group_by, subgroup_by, facet),
                               "ctdna_plot_reduction")
    if (!is.null(.pn))
      caption <- if (is.null(caption) || !nzchar(caption)) .pn
                 else paste0(caption, "\n", .pn)
  }
...
#' @param facet Optional column naming a panelling variable. One contingency
#'   table is tabulated per level and the panels are laid out with
#'   \code{facet_wrap}, e.g. \code{facet = "Indication"}. \code{NULL} (the
#'   default) draws a single pooled tile. The caption statistics remain pooled
#'   across the whole cohort.
#' @param wrap Alias for \code{facet}; both names are accepted. If both are
#'   supplied and they differ, \code{facet} wins and a message is emitted.
...
ctdna_concordance <- function(prep,
                              ...
                              wrap            = NULL) {
  # v0.79.0: `wrap` is an alias for `facet` (and vice versa).
  facet <- .ctdna_resolve_facet(facet, wrap, "ctdna_concordance")
  wrap  <- facet
  prep <- .ctdna_filter_prep_by_indication(prep, .ctdna_resolve_indication(indication, indications))
  ...
  # v0.79.0: optional panelling. `facet` (alias `wrap`) names a per-subject
  # column; one contingency table is tabulated per level and the panels are
  # laid out with facet_wrap. The caption statistics stay pooled -- they
  # describe the whole cohort, which is what the un-panelled plot reported.
  facet_col <- NULL
  if (!is.null(facet) && length(facet)) {
    if (length(facet) > 1L) {
      message("ctdna_concordance: only one panelling variable is supported; ",
              "using `", facet[1], "`.")
      facet <- facet[1]
    }
    facet_col <- .resolve_col(facet, df, what = "facet")
    fv <- df[[facet_col]][match(keep, as.character(df[[.o("subject")]]))]
    ok <- !is.na(fv)
    if (!any(ok)) {
      message("ctdna_concordance: `facet = \"", facet_col,
              "\"` is NA for every subject; ignoring.")
      facet_col <- NULL
    } else {
      parts <- lapply(split(seq_along(keep)[ok], droplevels(factor(fv[ok]))),
                      function(idx) {
                        tb <- table(setNames(list(droplevels(factor(yf[idx], levels = levels(yf))),
                                                  droplevels(factor(xf[idx], levels = levels(xf)))),
                                             c(ya$label, xa$label)))
                        as.data.frame(tb, stringsAsFactors = TRUE)
                      })
      long <- do.call(rbind, lapply(names(parts), function(nm) {
        d <- parts[[nm]]
        names(d)[1:2] <- c(ya$label, xa$label)
        d[[facet_col]] <- nm
        d
      }))
      rownames(long) <- NULL
  ...
  if (!is.null(facet_col))
    p <- p + ggplot2::facet_wrap(stats::as.formula(paste("~", sprintf("`%s`", facet_col))))
  # v0.80.0: say so on the figure when several indications are pooled
  .pn <- .ctdna_pooled_note(prep, c(x, y, facet), "ctdna_concordance")
  if (!is.null(.pn)) caption <- if (is.null(caption) || !nzchar(caption)) .pn else paste0(caption, "\n", .pn)
...
#' @param wrap Alias for \code{facet}; both names are accepted on every
#'   function that supports panelling. If both are supplied and they
#'   differ, \code{facet} wins and a message is emitted.
...
#' @param xlim Optional numeric x-limits (\code{c(lo, hi)}). \code{NULL}
#'   (default) uses the data range.
...
#'   \code{c("NSCLC","mCRPC")}). When set, restricts the analysis to
#'   patients whose Indication is in this set. Lookup falls through
#'   Indication -> indication -> Cancertype -> Cancer_Type. \code{NULL}
#'   (default) = no filter.
...
ctdna_plot_longitudinal <- function(prep,
                              ...
                              indications = NULL) {
  # v0.79.0: `wrap` is an alias for `facet` (and vice versa).
  facet <- .ctdna_resolve_facet(facet, wrap, "ctdna_plot_longitudinal")
  wrap  <- facet
  ...
  .pn <- .ctdna_pooled_note(prep, c(group_by, facet), "ctdna_plot_longitudinal")
...
#' @param xlim,ylim Optional numeric \code{c(lo, hi)} axis limits.
#'   \code{NULL} (default) uses the data range; when both are
#'   \code{NULL} and the two axes share units (both percentage or
#'   both ratio), the scatter is drawn on a squared coordinate box.
...
ctdna_plot_scatter <- function(prep,
                               ...
                               indications = NULL) {
  # v0.79.0: `wrap` is an alias for `facet` (and vice versa).
  facet <- .ctdna_resolve_facet(facet, wrap, "ctdna_plot_scatter")
  wrap  <- facet
  ...
  # v0.80.0: say so on the figure when several indications are pooled
  .pn <- .ctdna_pooled_note(prep, c(color_by, shape_by, facet), "ctdna_plot_scatter")
  if (!is.null(.pn)) caption <- if (is.null(caption) || !nzchar(caption)) .pn else paste0(caption, "\n", .pn)
...
#' @param y_clip Numeric \code{c(lo, hi)} used to bound the y-axis
#'   view when the data has extreme outliers (a single
#'   \eqn{+10{,}000\%} row would otherwise dominate the scale).
#'   Default \code{c(-100, 200)}. Pass \code{NULL} to render the
#'   full unclipped range. Overridden by an explicit \code{ylim}.
#' @param xlim,ylim Optional numeric \code{c(lo, hi)} axis limits.
#'   \code{NULL} (default) uses the data range for \code{xlim} and
#'   \code{y_clip} for \code{ylim}.
...
#' @examples
#' sim  <- ctdna_make_mock_study(n_patients = 20, seed = 1)
#' # Passing tf_change to ctdna_prepare() populates the required
#' # Ctdna_percentage_change column on prep$samples.
#' prep <- ctdna_prepare(
#'   infinity_report = sim$infinity_report,
...
#' p <- ctdna_plot_pct_change_by_dose_time(prep$samples,
...
ctdna_plot_pct_change_by_dose_time <- function(
    ...
    indications = NULL) {
  # ---- indications filter (v0.75.10) ----
  df <- .ctdna_filter_df_by_indication(df, .ctdna_resolve_indication(indication, indications))
  # ---------------------------------------
  if (!is.data.frame(df))
  ...
  .default_ylab <- if (identical(value_col, "Ctdna_percentage_change"))
    "Percent change from baseline (%)" else value_col
  .finalize(p,
...
#' @param xlim Optional character vector fixing the x-axis level order,
#'   or numeric \code{c(lo, hi)}. \code{NULL} (default) = data-driven.
#' @param show_stats,show_n Display toggles.
#' @param title,subtitle,caption,xlab,ylab,point_size,point_alpha,legend_position Display.
#' @param indication Optional indication (cohort) restriction. A single string
#'   or a character vector of Indication values, e.g.
...
#'
#' # methylTF by RECIST.
#' p1 <- ctdna_boxplot(prep, y = "methyl", x = "RECIST")
#' print(p1)
#'
#' # Any numeric column: TMB by dose.
#' p2 <- ctdna_boxplot(prep, y = "TMB_score", x = "Dose")
#' print(p2)
#'
#' # Two-factor split at a specific visit -- Baseline is the canonical
#' # baseline-visit label after tf_change is passed to prepare()
#' # (the vendor "C1D1" label is normalised to "Baseline").
#' p3 <- ctdna_boxplot(prep, y = "methyl", x = "Dose",
#'   color_by = "RECIST", visit = "Baseline")
#' print(p3)
#'
#' # ctDNA percent change from baseline at a specific on-treatment visit.
#' p4 <- ctdna_boxplot(prep, y = "ctdna_change", x = "RECIST",
#'   visit = "C2D1")
#' print(p4)
#' @export
ctdna_boxplot <- function(prep,
                          ...
                          indications = NULL) {
  # v0.79.0: `wrap` is an alias for `facet` (and vice versa).
  facet <- .ctdna_resolve_facet(facet, wrap, "ctdna_boxplot")
  wrap  <- facet
  ...
  if (isTRUE(show_stats)) {
    ...
    {
      # v0.80.0: say so on the figure when several indications are pooled
      .pn <- .ctdna_pooled_note(prep, c(xcol, ccol, facet), "ctdna_boxplot")
      if (!is.null(.pn))
        caption <- if (is.null(caption) || !nzchar(caption)) .pn
                   else paste0(caption, "\n", .pn)
    }
